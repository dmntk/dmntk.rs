...
fn test_node_between() {
    ...
    eqd(
        r#"Between(Name(Name("x")), Numeric("1", ""), Numeric("10", ""))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_comma_list() {
    ...
    eqd(
        r#"CommaList([Numeric("1", ""), Numeric("2", ""), Numeric("3", "")])"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_context() {
    let node = &AstNode::Context(vec![
        AstNode::ContextEntry(
            Box::new(AstNode::ContextEntryKey(s!("count").into())),
            Box::new(AstNode::Numeric(s!(1), s!())),
        ),
        AstNode::ContextEntry(
...
fn test_node_context_entry() {
    let node = &AstNode::ContextEntry(
        Box::new(AstNode::ContextEntryKey(s!("count").into())),
        Box::new(AstNode::Numeric(s!(1), s!())),
    );
    eqd(
        r#"ContextEntry(ContextEntryKey(Name("count")), Numeric("1", ""))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_context_type_entry() {
    ...
    eqd(
        r#"ContextTypeEntry(ContextTypeEntryKey(Name("count")), FeelType(Number))"#,
        node,
    );
    eqs(
        r#"
...
#[test]
fn test_node_div() {
    let node = &AstNode::Div(
        Box::new(AstNode::Numeric(s!(1), s!(23))),
        Box::new(AstNode::Numeric(s!(2), s!(34))),
    );
...
fn test_node_eq() {
    let node = &AstNode::Eq(
        Box::new(AstNode::String(s!("a"))),
        Box::new(AstNode::String(s!("b"))),
    );
...
fn test_node_filter() {
    let node = &AstNode::Filter(
        Box::new(AstNode::List(vec![
            AstNode::Numeric(s!(1), s!()),
            AstNode::Numeric(s!(2), s!()),
        ])),
        Box::new(AstNode::Gt(
...
fn test_node_for() {
    let node = &AstNode::For(
        Box::new(AstNode::Name(s!("i").into())),
        Box::new(AstNode::List(vec![_num!(1), _num!(2)])),
    );
    eqd(
        r#"For(Name(Name("i")), List([Numeric("1", ""), Numeric("2", "")]))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_formal_parameter() {
    let node = &AstNode::FormalParameter(
        Box::new(AstNode::Name(s!("amount").into())),
        Box::new(AstNode::FeelType(FeelType::Number)),
    );
    eqd(
        r#"FormalParameter(Name(Name("amount")), FeelType(Number))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_function_definition() {
    let node = &AstNode::FunctionDefinition(
        ...
        Box::new(AstNode::FunctionBody(b_num!(1), false)),
    );
    eqd(
        r#"FunctionDefinition(FormalParameters([]), FunctionBody(Numeric("1", ""), false))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_function_invocation() {
    ...
    eqd(
        r#"FunctionInvocation(Name(Name("calculate")), FormalParameters([]))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_function_type() {
    let node = &AstNode::FunctionType(
        Box::new(AstNode::ParameterTypes(vec![AstNode::FeelType(
            FeelType::Number,
        )])),
        Box::new(AstNode::FeelType(FeelType::Boolean)),
    );
    eqd(
        r#"FunctionType(ParameterTypes([FeelType(Number)]), FeelType(Boolean))"#,
        node,
    );
    eqs(
        r#"
    ...
    eqd(
        r#"FunctionType(FeelType(Boolean), FeelType(Boolean))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_if() {
    let node = &AstNode::If(
        Box::new(AstNode::Gt(b_num!(1), b_num!(2))),
        b_bool!(true),
        b_bool!(false),
    );
    eqd(
        r#"If(Gt(Numeric("1", ""), Numeric("2", "")), Boolean(true), Boolean(false))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_in() {
    let node = &AstNode::In(
        b_num!(1),
        Box::new(AstNode::List(vec![_num!(1), _num!(2), _num!(3)])),
    );
    eqd(
...
fn test_node_iteration_contexts() {
    let node = &AstNode::IterationContexts(vec![
        AstNode::Range(b_num!(1), b_num!(10)),
        AstNode::Range(b_num!(100), b_num!(110)),
    ]);
    eqd(
...
fn test_node_iteration_context_single() {
    let node = &AstNode::IterationContextSingle(
        b_name!(i),
        Box::new(AstNode::List(vec![_num!(1), _num!(2), _num!(3)])),
    );
    eqd(
...
fn test_node_iteration_context_range() {
    ...
    eqd(
        r#"IterationContextRange(Name(Name("i")), Numeric("1", ""), Numeric("10", ""))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_list() {
    ...
    eqd(
        r#"List([Numeric("1", ""), Numeric("2", ""), Numeric("3", "")])"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_named_parameter() {
    let node = &AstNode::NamedParameter(
        Box::new(AstNode::Name(s!("count").into())),
        Box::new(AstNode::FeelType(FeelType::Number)),
    );
    eqd(
        r#"NamedParameter(Name(Name("count")), FeelType(Number))"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_named_parameters() {
    ...
    eqd(
        r#"NamedParameters([NamedParameter(Name(Name("count")), FeelType(Number))])"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_positional_parameters() {
    ...
    eqd(
        r#"PositionalParameters([Numeric("1", ""), Numeric("2", "")])"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_qualified_name() {
    ...
    eqd(
        r#"QualifiedName([Name(Name("a")), Name(Name("b")), Name(Name("c"))])"#,
        node,
    );
    eqs(
        r#"
...
fn test_node_quantified_context() {
    ...
    eqd(
        r#"QuantifiedContext(Name(Name("a")), Numeric("10", ""))"#,
        node,
    );
    eqs(
        r#"
