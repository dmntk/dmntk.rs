...
fn _0001() {
    ...
    let node = parse_context(
        &scope,
        r#" { x: 10, a: function(a) function(b) a * b + x } "#,
        false,
    )
    .unwrap();
...
fn _0002() {
    ...
    .unwrap();
    assert_eq!(
        r#"Closure({QualifiedName([Name("x")])})"#,
        format!("{:?}", ClosureBuilder::from_node(&node))
    );
}
...
#[test]
fn _0017() {
    let node = parse_textual_expression(
        &scope!(),
        r#" "a" instance of function<string,string>->string "#,
        false,
    )
    .unwrap();
    assert_eq!("[]", format!("{}", ClosureBuilder::from_node(&node)));
...
fn _0018() {
    let node = parse_textual_expression(
        &scope!(),
        r#" ["a","b","c"] instance of list<string> "#,
        false,
    )
    .unwrap();
    assert_eq!("[]", format!("{}", ClosureBuilder::from_node(&node)));
...
#[test]
fn _0019() {
    let node = parse_textual_expression(&scope!(), r#" [1..10] instance of range<number> "#, false)
        .unwrap();
    assert_eq!("[]", format!("{}", ClosureBuilder::from_node(&node)));
}
...
fn _0021() {
    let node = AstNode::Out(
        Box::new(AstNode::Numeric("1".into(), "2".into())),
        Box::new(AstNode::Numeric("1".into(), "2".into())),
    );
    assert_eq!("[]", format!("{}", ClosureBuilder::from_node(&node)));
}
