...
fn ast_node_to_tree(node: &AstNode) -> AsciiNode {
    match node {
        ...
        AstNode::FunctionBody(lhs, external) => {
            node_and_label("FunctionBody", lhs, " (external)", "", *external)
        }
        ...
        AstNode::IntervalEnd(lhs, closed) => {
            node_and_label("IntervalEnd", lhs, " (closed)", " (opened)", *closed)
        }
        AstNode::IntervalStart(lhs, closed) => {
            node_and_label("IntervalStart", lhs, " (closed)", " (opened)", *closed)
        }
        ...
        AstNode::IterationContextRange(lhs, mid, rhs) => {
            node_3("IterationContextRange", lhs, mid, rhs)
        }
    ...
    if items.is_empty() {
        node_builder.add_child(
            AsciiNode::leaf_builder()
                .line(AsciiLine::builder().text("(empty)").build())
                .build(),
        );
    } else {
        for item in items {
...
fn node_and_leaf(name: &str, leaf: &str) -> AsciiNode {
    AsciiNode::node_builder(AsciiLine::builder().text(name).build())
        .child(
            AsciiNode::leaf_builder()
                .line(AsciiLine::builder().text(leaf).build())
                .build(),
        )
        .build()
}
