...
impl Canvas {
    ...
    fn recognize_region(&mut self, layer: Layer, top_left: Point) -> Result<Rect> {
        ...
        self.search_right(layer, &CORNERS_TOP_RIGHT, &['─', '┴'])
            .and_then(|_| {
                // move down to the nearest bottom-right corner
                self.search_down(layer, &CORNERS_BOTTOM_RIGHT, &['│', '├'])
                    .and_then(|(_, bottom_right)| {
                        // move left to the nearest bottom-left corner
                        self.search_left(layer, &CORNERS_BOTTOM_LEFT, &['─', '┬'])
                            .and_then(|_| {
                                // move up to the nearest top-left corner
                                self.search_up(layer, &CORNERS_TOP_LEFT, &['│', '┤'])
                                    .and_then(|(_, closing)| {
                                        self.close_rectangle(closing, top_left, bottom_right)
                                    })
                            })
                    })
            })
