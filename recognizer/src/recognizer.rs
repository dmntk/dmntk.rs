    ...
    fn recognize_horizontal_table(&mut self) -> Result<()> {
        ...
        let allowed_values_present = match r_in.height() {
            ...
            2 => {
                ...
                !self.plane.equal_regions_in_columns(&r_in)?
                    || (self.input_clause_count == 0
                        && self.output_clause_count > 1
                        && !self.plane.equal_regions_in_columns(&r_out)?)
            }
            3 => {
        ...
        if allowed_values_present {
            for col in r_in.left..r_in.right {
                self.allowed_input_values
                    .push(self.opt_text(self.plane.region_text(r_in.bottom - 1, col)?));
            }
        }
        ...
        for row in r.top..r.bottom {
            ...
            for col in r.left..r.right {
                self.input_entries
                    .last_mut()
                    .unwrap()
                    .push(self.plane.region_text(row, col)?);
            }
        }
            ...
            1 => {
                ...
                match r_out.height() {
                    1 => {
                        ...
                        self.output_label =
                            self.opt_text(self.plane.region_text(r_out.top, r_out.left)?);
                    }
                    2 => {
                        ...
                        self.allowed_output_values
                            .push(self.opt_text(self.plane.region_text(r_out.top + 1, r_out.left)?))
                    }
            ...
            _ => {
                ...
                match r_out.height() {
                    1 => {
                        ...
                        for col in r_out.left..r_out.right {
                            self.output_components
                                .push(self.opt_text(self.plane.region_text(r_out.top, col)?));
                        }
                    }
                    2 => {
                        if allowed_values_present {
                            ...
                            for col in r_out.left..r_out.right {
                                self.allowed_output_values.push(
                                    self.opt_text(self.plane.region_text(r_out.top + 1, col)?),
                                );
                            }
                        } else {
                    ...
                    3 => {
                        ...
                        for col in r_out.left..r_out.right {
                            self.output_components
                                .push(self.opt_text(self.plane.region_text(r_out.top + 1, col)?));
                        }
                        for col in r_out.left..r_out.right {
                            self.allowed_output_values
                                .push(self.opt_text(self.plane.region_text(r_out.top + 2, col)?));
                        }
                    }
                    _ => return Err(err_too_many_rows_in_output_clause()),
                }
