...
/// defined in [Technology Compatibility Kit for DMN standard](https://github.com/dmn-tck/tck).
#[post("/tck")]
pub async fn post_tck_evaluate(
    params: Json<TckEvaluateParams>,
    data: web::Data<ApplicationData>,
) -> io::Result<Json<TckResultDto<OutputNodeDto>>> {
...
fn do_evaluate_tck(
    ...
    params: TckEvaluateParams,
) -> Result<OutputNodeDto, DmntkError> {
...
fn prepare_output_node_dto(value: Value) -> OutputNodeDto {
    match ValueDto::try_from(&value) {
        Ok(value_dto) => OutputNodeDto {
            value: Some(value_dto),
        },
