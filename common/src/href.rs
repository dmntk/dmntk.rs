...
impl TryFrom<&str> for HRef {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if let Ok(reference) = RelativeReference::try_from(value) {
            ...
            let id = reference
                .fragment()
                .ok_or_else(|| err_invalid_reference_no_fragment(value))?
                .to_string();
            let path = reference
                .path()
                .to_string()
...
mod tests {
    ...
    fn test_relative_references() {
        assert_eq!(
            r#"Err(DmntkError("<HRefError> no fragment in reference: ''"))"#,
        ...
        assert_eq!(
            ...
            format!(
                "{:?}",
                HRef::try_from("documents#_b51ac78b-fd76-42fc-a12d-aad7150c9278")
            )
        );
        assert_eq!(
            ...
            format!(
                "{:?}",
                HRef::try_from("#_b51ac78b-fd76-42fc-a12d-aad7150c9278")
            )
        );
        assert_eq!(
            ...
            format!(
                "{:?}",
                HRef::try_from("documents?name=Introduction#_b51ac78b-fd76-42fc-a12d-aad7150c9278")
            )
        );
    }
    ...
    fn test_absolute_references() {
        ...
        assert_eq!(
            ...
            format!(
                "{:?}",
                HRef::try_from("https://dmntk.io/documents#_b51ac78b-fd76-42fc-a12d-aad7150c9278")
            )
        );
        assert_eq!(
