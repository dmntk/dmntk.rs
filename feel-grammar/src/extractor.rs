...
pub fn extract(input: &str) -> String {
    ...
    // generate actions
    {
        ...
        for (num, comment, rule) in &actions {
            let _ = writeln!(
                output,
                "    {num} => reduce_actions.action_{rule}(), // {comment}"
            );
        }
...
fn extract_token_types(input: &str) -> Vec<(String, i64)> {
    ...
    let re_token_types: Regex =
        Regex::new("(?m).*enum\\s+yytokentype[^{]+\\{(?P<items>[^}]+)};.*").unwrap();
    let re_token_type: Regex =
        Regex::new("TOKEN_(?P<name>[A-Za-z_]+)\\s+=\\s+(?P<value>-?[0-9]+)").unwrap();
...
fn extract_symbol_kinds(input: &str) -> Vec<(String, i64)> {
    ...
    let re_symbol_kinds: Regex =
        Regex::new("(?m).*enum\\s+yysymbol_kind_t[^{]+\\{(?P<items>[^}]+)};.*").unwrap();
    let re_symbol_kind: Regex =
        Regex::new("SYMBOL_(?P<name>[A-Za-z_]+)\\s+=\\s+(?P<value>-?[0-9]+)").unwrap();
...
fn extract_numeric_table(input: &str, table_name: &str) -> (String, Vec<i64>) {
    ...
    if let Some(captures) = re.captures(input) {
        ...
        let table_type = match type_match.as_str() {
            ...
            _ => {
                panic!(
                    "unhandled type for table elements `{}`",
                    type_match.as_str()
                );
            }
        }
        ...
        return (
            table_type,
            s.map(|a| a.trim().parse::<i64>().unwrap()).collect(),
        );
    }
...
fn extract_semantic_actions(input: &str) -> Vec<(i64, String, String)> {
    ...
    let re_actions: Regex =
        Regex::new("(?m).*\\s+switch\\s+\\(yyn\\)[^{]+\\{(?P<actions>[^}]+)}.*").unwrap();
    let re_action: Regex = Regex::new("(?m)case\\s+[0-9]+:\\s+/\\*[^*]+\\*/[^;]+").unwrap();
    let re: Regex = Regex::new(
        "(?m)case\\s+(?P<num>[0-9]+):\\s+/\\*(?P<comment>[^*]+)\\*/[^#]+#(?P<rule>[^@]+)",
    )
    .unwrap();
