...
fn set_file_permissions(file_name: &str) {
    ...
    fs::set_permissions(file_name, Permissions::from_mode(0o755))
        .expect("setting script permissions failed");
}
...
fn lalr_c_tables() -> String {
    {
        ...
        let mut grammar_file =
            File::create(grammar_file_name).expect("creating grammar file failed");
        grammar_file
            .write_all(FEEL_GRAMMAR.as_bytes())
            .expect("writing grammar file failed");
        ...
        script_file
            .write_all(GEN_SCRIPT.as_bytes())
            .expect("writing script file failed");
    ...
    }
    {
        ...
        command_process
            .wait()
            .expect("waiting for command process failed");
    }
    fs::read_to_string(format!("{TARGET_DIR}/{TABLES_FILE_NAME}"))
        .expect("generating parsing tables failed")
}
