    ...
    match get_matches().subcommand() {
        ...
        Some(("pfe", matches)) => {
            return Action::ParseFeelExpression(
                matches
                    .get_one::<String>("CONTEXT_FILE")
                    .unwrap_or(&DEFAULT_CTX)
                    .to_string(),
                matches
                    .get_one::<String>("FEEL_FILE")
                    .unwrap_or(&DEFAULT_FEEL)
                    .to_string(),
                matches
        ...
        Some(("pdt", matches)) => {
            return Action::ParseDecisionTable(
                matches
                    .get_one::<String>("DECTAB_FILE")
                    .unwrap_or(&DEFAULT_DTB)
                    .to_string(),
            );
        }
        ...
        Some(("rdt", matches)) => {
            return Action::RecognizeDecisionTable(
                matches
                    .get_one::<String>("DECTAB_FILE")
                    .unwrap_or(&DEFAULT_DTB)
                    .to_string(),
        ...
        Some(("pdm", matches)) => {
            return Action::ParseDmnModel(
                matches
                    .get_one::<String>("DMN_FILE")
                    .unwrap_or(&DEFAULT_DMN)
                    .to_string(),
                matches
                    .get_one::<String>("color")
                    .unwrap_or(&DEFAULT_COLOR)
        ...
        Some(("edm", matches)) => {
            return Action::EvaluateDmnModel(
                ...
                matches
                    ...
                    .to_string(),
                matches
                    .get_one::<String>("invocable")
                    .unwrap_or(&DEFAULT_INVOCABLE)
                    .to_string(),
            );
        }
        ...
        // generate examples
        Some(("exs", matches)) => {
            return Action::SaveExamples(
                matches
                    .get_one::<String>("DIR")
                    .unwrap_or(&DEFAULT_EXAMPLES_DIR)
                    .to_string(),
            );
        }
        _ => {}
...
fn parse_feel_expression(ctx_file_name: &str, feel_file_name: &str, color_mode: ColorMode) {
    match fs::read_to_string(feel_file_name) {
        Ok(feel_expression) => match fs::read_to_string(ctx_file_name) {
            Ok(context_definition) => {
                match dmntk_evaluator::evaluate_context(&FeelScope::default(), &context_definition)
                {
                    Ok(ctx) => match dmntk_feel_parser::parse_expression(
                        &ctx.into(),
                        &feel_expression,
                        false,
                    ) {
                        Ok(ast_root_node) => {
                            println!(
                                "    AST:{}",
                                ast_tree(&ast_root_node, &color_mode).trim_end()
                            );
                        }
            ...
            Ok(context_definition) => {
                match dmntk_evaluator::evaluate_context(&FeelScope::default(), &context_definition)
                {
                    Ok(ctx) => match dmntk_feel_parser::parse_expression(
                        ...
                        Ok(ast_root_node) => {
                            match dmntk_evaluator::evaluate(&ctx.into(), &ast_root_node) {
                                Ok(result) => {
                                    println!("{result}");
                                }
                                Err(reason) => {
                    ...
                    for (test_no, (input_data, expected)) in test_cases.iter().enumerate() {
                        ...
                        match dmntk_feel_parser::parse_expression(&scope, &feel_file_content, false)
                        {
                            Ok(node) => match dmntk_evaluator::evaluate(&scope, &node) {
                                Ok(actual) => display_test_case_result(
                                    &actual,
                                    expected,
                                    &test_no,
                                    &mut passed,
...
fn parse_decision_table(dectab_file_name: &str) {
    match fs::read_to_string(dectab_file_name) {
        ...
        Err(reason) => eprintln!(
            "loading decision table file `{dectab_file_name}` failed with reason: {reason}"
        ),
    }
}
...
fn evaluate_decision_table(input_file_name: &str, dectab_file_name: &str) {
    ...
    let input_data =
        match dmntk_evaluator::evaluate_context(&FeelScope::default(), &input_file_content) {
...
fn test_decision_table(
    ...
    for (test_no, (input_data, expected)) in test_cases.iter().enumerate() {
        let scope = input_data.clone().into();
        let evaluator =
            match dmntk_evaluator::build_decision_table_evaluator(&scope, &decision_table) {
                Ok(evaluator) => evaluator,
                Err(reason) => {
                    eprintln!("building decision table evaluator failed with reason: {reason}");
                    return;
...
fn export_decision_table(dectab_file_name: &str, html_file_name: &str) {
    match fs::read_to_string(dectab_file_name) {
        Ok(text) => {
            match dmntk_recognizer::recognize_decision_table(&text, false) {
                Ok(decision_table) => {
...
fn evaluate_dmn_model(input_file_name: &str, dmn_file_name: &str, invocable_name: &str) {
    match fs::read_to_string(dmn_file_name) {
        Ok(dmn_file_content) => match fs::read_to_string(input_file_name) {
                    ...
                    Ok(input_data) => match dmntk_model::parse(&dmn_file_content) {
                        Ok(definitions) => {
                            ...
                            match dmntk_evaluator::ModelEvaluator::new(&[definitions]) {
                                Ok(model_evaluator) => {
                                    let result = model_evaluator.evaluate_invocable(
                                        &namespace,
                                        invocable_name,
                                        &input_data,
                                    );
                                ...
                                Err(reason) => eprintln!(
                                    "building model evaluator failed with reason: {reason}"
                                ),
                            }
                        }
            ...
            Err(reason) => eprintln!(
                "loading input data file `{input_file_name}` failed with reason: {reason}"
            ),
        },
        Err(reason) => {
            eprintln!("loading model file `{dmn_file_name}` failed with reason: {reason}")
...
fn export_dmn_model(dmn_file_name: &str, html_file_name: &str) {
    match fs::read_to_string(dmn_file_name) {
        Ok(dmn_file_content) => {
            match dmntk_model::parse(&dmn_file_content) {
                Ok(definitions) => {
