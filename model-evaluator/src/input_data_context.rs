...
type InputDataContextEvaluatorFn =
    Box<dyn Fn(&mut FeelContext, &ItemDefinitionContextEvaluator) -> FeelType + Send + Sync>;
...
pub fn input_data_context_evaluator(input_data: &DefInputData) -> InputDataContextEvaluatorFn {
    ...
    if let Some(simple_type_ref) = type_ref_to_feel_type(&type_ref) {
        Box::new(
            move |ctx: &mut FeelContext, _: &ItemDefinitionContextEvaluator| {
        ...
        Box::new(
            move |ctx: &mut FeelContext, evaluator: &ItemDefinitionContextEvaluator| {
...
mod tests {
    ...
    fn build_evaluators(xml: &str) -> (InputDataContextEvaluator, ItemDefinitionContextEvaluator) {
        ...
        let item_definition_context_evaluator =
