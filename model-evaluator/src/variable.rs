...
pub type VariableEvaluatorFn =
    Box<dyn Fn(&Value, &ItemDefinitionEvaluator) -> (Name, Value) + Send + Sync>;
...
impl Variable {
    ...
    pub fn build_evaluator(&self) -> VariableEvaluatorFn {
        ...
        match variable_type_ref.as_str() {
                ...
                if let Value::Context(ctx) = value {
                    if let Some(v) = ctx.get_entry(&variable_name) {
                        return if let Value::DaysAndTimeDuration(_) = v {
                            ...
                            (
                                variable_name.clone(),
                                v.coerced(&FeelType::DaysAndTimeDuration),
                            )
                        };
                    }
                }
                ...
                if let Value::Context(ctx) = value {
                    if let Some(v) = ctx.get_entry(&variable_name) {
                        return if let Value::YearsAndMonthsDuration(_) = v {
                            ...
                            (
                                variable_name.clone(),
                                v.coerced(&FeelType::YearsAndMonthsDuration),
                            )
                        };
                    }
                }
            ...
            _ => Box::new(
                move |value: &Value, item_definition_evaluator: &ItemDefinitionEvaluator| {
                    if let Value::Context(ctx) = value {
              ...
              .unwrap_or_else(|| value_null!("input data evaluator: item definition evaluator '{}' not found", variable_type_ref));
                            ...
                            (
                                variable_name.clone(),
                                value_null!("no name {} in context {}", variable_name, ctx),
                            )
                        }
                    } else {
                        (
                            variable_name.clone(),
                            value_null!("expected context, actual value is {}", value),
                        )
                    }
                },
            ),
