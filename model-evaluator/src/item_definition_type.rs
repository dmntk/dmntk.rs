...
type ItemDefinitionTypeEvaluatorFn =
    Box<dyn Fn(&ItemDefinitionTypeEvaluator) -> Option<FeelType> + Send + Sync>;
...
///
pub fn build_item_definition_type_evaluator(
    item_definition: &DefItemDefinition,
) -> Result<ItemDefinitionTypeEvaluatorFn> {
    match item_definition.item_definition_type()? {
        ItemDefinitionType::SimpleType(feel_type) => simple_type(feel_type),
        ItemDefinitionType::ReferencedType(namespace, ref_type) => {
            referenced_type(DefKey::new(&namespace, &ref_type))
        }
        ItemDefinitionType::ComponentType => component_type(item_definition),
        ItemDefinitionType::CollectionOfSimpleType(feel_type) => {
            collection_of_simple_type(feel_type)
        }
        ItemDefinitionType::CollectionOfReferencedType(namespace, ref_type) => {
            collection_of_referenced_type(DefKey::new(&namespace, &ref_type))
        }
        ItemDefinitionType::CollectionOfComponentType => {
            collection_of_component_type(item_definition)
        }
...
fn simple_type(feel_type: FeelType) -> Result<ItemDefinitionTypeEvaluatorFn> {
    match feel_type {
        FeelType::Any => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::Any)
        })),
        FeelType::String => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::String)
        })),
        FeelType::Number => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::Number)
        })),
        FeelType::Boolean => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::Boolean)
        })),
        FeelType::Date => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::Date)
        })),
        FeelType::Time => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::Time)
        })),
        FeelType::DateTime => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::DateTime)
        })),
        FeelType::DaysAndTimeDuration => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::DaysAndTimeDuration)
        })),
        FeelType::YearsAndMonthsDuration => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::YearsAndMonthsDuration)
        })),
...
fn collection_of_simple_type(feel_type: FeelType) -> Result<ItemDefinitionTypeEvaluatorFn> {
    match feel_type {
        FeelType::Any => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::list(&FeelType::Any))
        })),
        FeelType::String => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::list(&FeelType::String))
        })),
        FeelType::Number => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::list(&FeelType::Number))
        })),
        FeelType::Boolean => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::list(&FeelType::Boolean))
        })),
        FeelType::Date => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::list(&FeelType::Date))
        })),
        FeelType::Time => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::list(&FeelType::Time))
        })),
        FeelType::DateTime => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
            Some(FeelType::list(&FeelType::DateTime))
        })),
        FeelType::DaysAndTimeDuration => Ok(Box::new(move |_: &ItemDefinitionTypeEvaluator| {
...
fn collection_of_referenced_type(def_key: DefKey) -> Result<ItemDefinitionTypeEvaluatorFn> {
    Ok(Box::new(move |evaluators: &ItemDefinitionTypeEvaluator| {
        evaluators
            .eval(&def_key)
            .map(|feel_type| FeelType::List(Box::new(feel_type)))
    }))
}
...
fn function_type(item_definition: &DefItemDefinition) -> Result<ItemDefinitionTypeEvaluatorFn> {
    ...
    Ok(Box::new(move |evaluator: &ItemDefinitionTypeEvaluator| {
        let output_type = evaluator
            .information_item_type(&namespace, &output_type_ref)
            .unwrap_or(FeelType::Any);
        let parameter_types = parameters_type_ref
            .iter()
            .map(|parameter_type_ref| {
                evaluator
                    .information_item_type(&namespace, parameter_type_ref)
                    .unwrap_or(FeelType::Any)
            })
...
mod tests {
    ...
    fn simple_type_string() {
        ...
        assert_eq!(
            Some(FeelType::String),
            evaluator.eval(&DefKey::new(NAMESPACE, "tCustomerName"))
        );
    }
    ...
    fn simple_type_number() {
        ...
        assert_eq!(
            Some(FeelType::Number),
            evaluator.eval(&DefKey::new(NAMESPACE, "tMonthlySalary"))
        );
    }
    ...
    fn simple_type_boolean() {
        ...
        assert_eq!(
            Some(FeelType::Boolean),
            evaluator.eval(&DefKey::new(NAMESPACE, "tIsAffordable"))
        );
    }
    ...
    fn simple_type_date() {
        ...
        assert_eq!(
            Some(FeelType::Date),
            evaluator.eval(&DefKey::new(NAMESPACE, "tBirthday"))
        );
    }
    ...
    fn simple_type_time() {
        ...
        assert_eq!(
            Some(FeelType::Time),
            evaluator.eval(&DefKey::new(NAMESPACE, "tDeliveryTime"))
        );
    }
    ...
    fn simple_type_date_time() {
        ...
        assert_eq!(
            Some(FeelType::DateTime),
            evaluator.eval(&DefKey::new(NAMESPACE, "tAppointment"))
        );
    }
    ...
    fn simple_type_days_and_time_duration() {
        ...
        assert_eq!(
            Some(FeelType::DaysAndTimeDuration),
            evaluator.eval(&DefKey::new(NAMESPACE, "tCourseDuration"))
        );
    }
    ...
    fn simple_type_years_and_month_duration() {
        ...
        assert_eq!(
            Some(FeelType::YearsAndMonthsDuration),
            evaluator.eval(&DefKey::new(NAMESPACE, "tGrowthDuration"))
        );
    }
    ...
    fn component_type() {
        ...
        let component_type = FeelType::context(&[
            (&name_principal, &type_number),
            (&name_rate, &type_number),
            (&name_term_months, &type_number),
        ]);
        assert_eq!(
            Some(component_type),
            evaluator.eval(&DefKey::new(NAMESPACE, "tLoan"))
        );
    }
    ...
    fn collection_of_simple_type_string() {
        ...
        assert_eq!(
            Some(FeelType::list(&FeelType::String)),
            evaluator.eval(&DefKey::new(NAMESPACE, "tItems"))
        );
    }
