...
macro_rules! model_evaluator_from_examples {
    ($model_name:tt) => {
        static MODEL_EVALUATOR: Lazy<Arc<ModelEvaluator>> =
            Lazy::new(|| build_model_evaluator(dmntk_examples::$model_name));
    };
}

macro_rules! model_namespace_from_examples {
    ($model_name:tt) => {
        static MODEL_NAMESPACE: Lazy<String> =
            Lazy::new(|| build_model_namespace(dmntk_examples::$model_name));
    };
}
...
macro_rules! model_evaluator {
    ($model_name:tt) => {
        static MODEL_EVALUATOR: Lazy<Arc<ModelEvaluator>> =
            Lazy::new(|| build_model_evaluator($model_name));
    };
}
...
use {
    from_examples, model_evaluator, model_evaluator_from_examples, model_namespace_from_examples,
    static_context,
};
    ...
    match dmntk_feel_parser::parse_context(&scope, input, false) {
        Ok(node) => match dmntk_feel_evaluator::prepare(&BuildContext::default(), &node) {
            Ok(evaluator) => match evaluator(&scope) {
                ...
                other => panic!(
                    "ERROR: expected context value, actual value is: {}",
                    other as Value
                ),
            },
...
fn assert_decision(
    ...
    let actual = model_evaluator
        .evaluate_invocable(namespace, invocable_name, input_data)
        .to_string();
    assert_eq!(
    expected, actual,
...
fn assert_business_knowledge_model(
    model_evaluator: &ModelEvaluator,
    namespace: &str,
    invocable_name: &str,
    input_data: &FeelContext,
