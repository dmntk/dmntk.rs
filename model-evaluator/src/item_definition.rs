...
pub type ItemDefinitionEvaluatorFn =
    Box<dyn Fn(&Value, &ItemDefinitionEvaluator) -> Value + Send + Sync>;
...
impl ItemDefinitionEvaluator {
    ...
    pub fn eval(&self, def_key: &DefKey, value: &Value) -> Option<Value> {
        self.evaluators
            .get(def_key)
            .map(|evaluator| evaluator(value, self))
    }
}

///
pub fn build_item_definition_evaluator(
    item_definition: &DefItemDefinition,
) -> Result<ItemDefinitionEvaluatorFn> {
    ...
    match item_definition.item_definition_type()? {
        ItemDefinitionType::SimpleType(feel_type) => {
            build_simple_type_evaluator(feel_type, av_evaluator)
        }
        ItemDefinitionType::ReferencedType(namespace, type_ref) => {
            build_referenced_type_evaluator(DefKey::new(&namespace, &type_ref))
        }
        ...
        ItemDefinitionType::CollectionOfSimpleType(feel_type) => {
            build_collection_of_simple_type_evaluator(feel_type, av_evaluator)
        }
        ItemDefinitionType::CollectionOfReferencedType(namespace, type_ref) => {
            build_collection_of_referenced_type_evaluator(
                DefKey::new(&namespace, &type_ref),
                av_evaluator,
            )
        }
        ItemDefinitionType::CollectionOfComponentType => {
            build_collection_of_component_type_evaluator(item_definition)
        }
...
///
fn build_allowed_values_evaluator(
    item_definition: &DefItemDefinition,
    ...
    if let Some(unary_tests) = item_definition.allowed_values() {
        if let Some(text) = unary_tests.text() {
            ...
            let node = AstNode::In(
                Box::new(AstNode::Name("?".into())),
                Box::new(unary_tests_node),
            );
            av_evaluator = Some(dmntk_feel_evaluator::prepare(
...
///
fn build_simple_type_evaluator(
    feel_type: FeelType,
    av_evaluator: Option<Evaluator>,
) -> Result<ItemDefinitionEvaluatorFn> {
    ///
    ...
    fn build_string_evaluator(av_evaluator: Option<Evaluator>) -> ItemDefinitionEvaluatorFn {
        Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| match value {
                Value::String(_) => check_allowed_values(value.to_owned(), av_evaluator.as_ref()),
                null @ Value::Null(_) => null.clone(),
                _ => value_null!(
                    "expected type 'string', actual type is '{}' in value '{}'",
                    value.type_of(),
                    value
                ),
            },
    ...
    fn build_number_evaluator(av_evaluator: Option<Evaluator>) -> ItemDefinitionEvaluatorFn {
        Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| match value {
                ...
                null @ Value::Null(_) => null.clone(),
                _ => value_null!(
                    "expected type 'number', actual type is '{}' in value '{}'",
                    value.type_of(),
                    value
                ),
            },
    ...
    fn build_boolean_evaluator(av_evaluator: Option<Evaluator>) -> ItemDefinitionEvaluatorFn {
        Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| match value {
                ...
                null @ Value::Null(_) => null.clone(),
                _ => value_null!(
                    "expected type 'boolean', actual type is '{}' in value '{}'",
                    value.type_of(),
                    value
                ),
            },
    ...
    fn build_date_evaluator(av_evaluator: Option<Evaluator>) -> ItemDefinitionEvaluatorFn {
        Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| match value {
                ...
                null @ Value::Null(_) => null.clone(),
                _ => value_null!(
                    "expected type 'date', actual type is '{}' in value '{}'",
                    value.type_of(),
                    value
                ),
            },
    ...
    fn build_time_evaluator(av_evaluator: Option<Evaluator>) -> ItemDefinitionEvaluatorFn {
        Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| match value {
                ...
                null @ Value::Null(_) => null.clone(),
                _ => value_null!(
                    "expected type 'time', actual type is '{}' in value '{}'",
                    value.type_of(),
                    value
                ),
            },
    ...
    fn build_date_time_evaluator(av_evaluator: Option<Evaluator>) -> ItemDefinitionEvaluatorFn {
        Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| match value {
                ...
                null @ Value::Null(_) => null.clone(),
                _ => value_null!(
                    "expected type 'date and time', actual type is '{}' in value '{}'",
                    value.type_of(),
                    value
                ),
            },
...
fn build_referenced_type_evaluator(def_key: DefKey) -> Result<ItemDefinitionEvaluatorFn> {
    Ok(Box::new(
        move |value: &Value, evaluators: &ItemDefinitionEvaluator| {
            evaluators
                .eval(&def_key, value)
                .unwrap_or_else(|| value_null!("no evaluator"))
        },
    ))
}

///
fn build_component_type_evaluator(
    item_definition: &DefItemDefinition,
) -> Result<ItemDefinitionEvaluatorFn> {
            ...
            if let Value::Context(ctx) = value {
                ...
                for (component_name, component_evaluator) in &component_evaluators {
                    if let Some(component_value) = ctx.get_entry(component_name) {
                        evaluated_ctx.set_entry(
                            component_name,
                            component_evaluator(component_value, evaluators),
                        );
                    } else {
...
///
fn build_collection_of_simple_type_evaluator(
    feel_type: FeelType,
    av_evaluator: Option<Evaluator>,
) -> Result<ItemDefinitionEvaluatorFn> {
    ///
    fn build_any_evaluator(av_evaluator: Option<Evaluator>) -> Result<ItemDefinitionEvaluatorFn> {
        Ok(Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| {
                ...
                if let Value::List(values) = value {
                    ...
                    for item_value in values {
                        if let Value::Date(_) = item_value {
                            ...
                            return value_null!(
                                "item definition evaluator (CollectionOfSimpleType): expected date"
                            );
                        }
                    }
                    check_allowed_values(Value::List(evaluated_values), av_evaluator.as_ref())
                ...
                if let Value::List(values) = value {
                    ...
                    for item_value in values {
                        if let Value::Time(_) = item_value {
                            ...
                            return value_null!(
                                "item definition evaluator (CollectionOfSimpleType): expected time"
                            );
                        }
                    }
                    check_allowed_values(Value::List(evaluated_values), av_evaluator.as_ref())
    ...
    ///
    fn build_date_and_time_evaluator(
        av_evaluator: Option<Evaluator>,
    ) -> Result<ItemDefinitionEvaluatorFn> {
        Ok(Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| {
    ...
    ///
    fn build_days_and_time_duration_evaluator(
        av_evaluator: Option<Evaluator>,
    ) -> Result<ItemDefinitionEvaluatorFn> {
        Ok(Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| {
    ...
    ///
    fn build_months_and_years_duration_evaluator(
        av_evaluator: Option<Evaluator>,
    ) -> Result<ItemDefinitionEvaluatorFn> {
        Ok(Box::new(
            move |value: &Value, _: &ItemDefinitionEvaluator| {
...
///
fn build_collection_of_referenced_type_evaluator(
    def_key: DefKey,
    av_evaluator: Option<Evaluator>,
) -> Result<ItemDefinitionEvaluatorFn> {
    Ok(Box::new(
            ...
            if let Value::List(values) = value {
                ...
                for item_value in values {
                    if let Some(evaluated_value) = evaluators.eval(&def_key, item_value) {
                        ...
                        return value_null!(
                            "no evaluator defined for type reference '{}'",
                            def_key
                        );
                    }
                }
                ...
                value_null!(
                    "expected list, actual type is '{}' in value '{}'",
                    value.type_of(),
                    value
                )
            }
...
///
fn build_collection_of_component_type_evaluator(
    item_definition: &DefItemDefinition,
) -> Result<ItemDefinitionEvaluatorFn> {
    let mut component_evaluators: Vec<(Name, ItemDefinitionEvaluatorFn)> = vec![];
    for component_item_definition in item_definition.item_components() {
            ...
            if let Value::List(values) = value {
                ...
                for item_value in values {
                    if let Value::Context(ctx) = item_value {
                        ...
                        return value_null!(
                            "expected context, actual type is '{}' in value '{}'",
                            item_value.type_of(),
                            item_value
                        );
                    }
...
mod tests {
    ...
    fn test_evaluate_input_data_0101_1() {
        ...
        let context =
            dmntk_feel_evaluator::evaluate_context(&Default::default(), context_str).unwrap();
        let value = context
            .get_entry(&Name::new(&["Customer", "Name"]))
            .unwrap();
        assert_eq!(
            Value::String("Whistler".to_string()),
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tCustomerName"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0102_1() {
        ...
        let context =
            dmntk_feel_evaluator::evaluate_context(&Default::default(), context_str).unwrap();
        let value = context
            .get_entry(&Name::new(&["Monthly", "Salary"]))
            .unwrap();
        assert_eq!(
            value_number!(12_000),
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tMonthlySalary"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0103_1() {
        ...
        let context =
            dmntk_feel_evaluator::evaluate_context(&Default::default(), context_str).unwrap();
        let value = context
            .get_entry(&Name::new(&["Is", "Affordable"]))
            .unwrap();
        assert_eq!(
            Value::Boolean(true),
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tIsAffordable"), value)
                .unwrap()
        );
    ...
    fn test_evaluate_input_data_0104_1() {
        ...
        assert_eq!(
            ...
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tBirthday"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0105_1() {
        ...
        let context =
            dmntk_feel_evaluator::evaluate_context(&Default::default(), context_str).unwrap();
        let value = context
            .get_entry(&Name::new(&["Delivery", "Time"]))
            .unwrap();
        assert_eq!(
            Value::Time(FeelTime::local_opt(18, 35, 23, 0).unwrap()),
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tDeliveryTime"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0106_1() {
        ...
        assert_eq!(
            Value::DateTime(FeelDateTime::new(
                FeelDate::new(2021, 10, 12),
                FeelTime::local_opt(18, 35, 23, 0).unwrap()
            )),
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tAppointment"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0107_1() {
        ...
        let context =
            dmntk_feel_evaluator::evaluate_context(&Default::default(), context_str).unwrap();
        let value = context
            .get_entry(&Name::new(&["Course", "Duration"]))
            .unwrap();
        assert_eq!(
            Value::DaysAndTimeDuration(FeelDaysAndTimeDuration::from_s(183600)),
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tCourseDuration"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0108_1() {
        ...
        let context =
            dmntk_feel_evaluator::evaluate_context(&Default::default(), context_str).unwrap();
        let value = context
            .get_entry(&Name::new(&["Growth", "Duration"]))
            .unwrap();
        assert_eq!(
            Value::YearsAndMonthsDuration(FeelYearsAndMonthsDuration::from_ym(2, 5)),
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tGrowthDuration"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0301_1() {
        ...
        assert_eq!(
            expected,
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tLoan"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0401_1() {
        ...
        assert_eq!(
            expected,
            evaluator
                .eval(&DefKey::new(NAMESPACE, "tItems"), value)
                .unwrap()
        );
    }
    ...
    fn test_evaluate_input_data_0404_1() {
        ...
        let context_str =
            r#"{ Items : [date("2021-10-10"), date("2021-10-11"), date("2021-10-12")] }"#;
        let context =
            dmntk_feel_evaluator::evaluate_context(&Default::default(), context_str).unwrap();
        let value = context.get_entry(&Name::new(&["Items"])).unwrap();
        let expected = Value::List(vec![
    ...
    fn test_evaluate_input_data_0406_1() {
        ...
        let expected = Value::List(vec![
            Value::DateTime(FeelDateTime::new(
                FeelDate::new(2021, 10, 10),
                FeelTime::local_opt(21, 23, 18, 0).unwrap(),
            )),
            Value::DateTime(FeelDateTime::new(
                FeelDate::new(2021, 10, 11),
                FeelTime::local_opt(12, 18, 59, 0).unwrap(),
            )),
        ]);
        assert_eq!(
            expected,
    ...
    fn test_evaluate_input_data_0407_1() {
        ...
        let context =
            dmntk_feel_evaluator::evaluate_context(&Default::default(), context_str).unwrap();
        let value = context.get_entry(&Name::new(&["Items"])).unwrap();
        let expected = Value::List(vec![Value::DaysAndTimeDuration(
            FeelDaysAndTimeDuration::from_s(183600),
        )]);
        assert_eq!(
            expected,
            evaluator
    ...
    fn test_evaluate_input_data_0601_1() {
        ...
        assert_eq!(
            Some(expected),
            evaluator.eval(&DefKey::new(NAMESPACE, "tItems"), value)
        );
    }
}
