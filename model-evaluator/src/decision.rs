...
///
type DecisionEvaluatorFn = Box<
    dyn Fn(&FeelContext, &FeelContext, &ModelEvaluator, &mut FeelContext) -> Name + Send + Sync,
>;
...
impl DecisionEvaluator {
    ...
    pub fn evaluate(
        ...
        self.evaluators.get(def_key).map(|evaluator_entry| {
            evaluator_entry.1(global_context, input_data, model_evaluator, evaluated_ctx)
        })
    }
...
fn build_decision_evaluator(
    ...
    def_decision: &DefDecision,
    model_builder: &ModelBuilder,
) -> Result<DecisionEvaluatorEntry> {
    ...
    bring_knowledge_requirements_into_context(
        def_definitions,
        def_decision.knowledge_requirements(),
        &mut build_requirements_ctx,
    )?;
    ...
    for information_requirement in def_decision.information_requirements() {
        ...
        if let Some(def_href) = information_requirement.required_decision() {
            if let Some(required_def_decision) =
                def_definitions.decision_by_key(def_href.namespace(), def_href.id())
            {
                ...
                let variable_type = item_definition_context_evaluator.eval(
                    &DefKey::new(variable_namespace, variable_type_ref),
                    &variable_name,
                    &mut build_requirements_ctx,
                );
                if let Some(import_name) = def_href.import_name() {
                    build_requirements_ctx.create_entries(
                        &[import_name.clone(), variable_name],
                        Value::FeelType(variable_type),
                    );
                } else {
                    build_requirements_ctx
                        .set_entry(&variable_name, Value::FeelType(variable_type));
                }
            }
        }
        ...
        if let Some(href) = information_requirement.required_input() {
            if let Some(required_input) =
                def_definitions.input_data_by_key(href.namespace(), href.id())
            {
                let variable_name = required_input.variable().name();
                let variable_type = input_data_context_evaluator.eval(
                    &href.into(),
                    &mut input_requirements_ctx,
    ...
    let evaluator = if let Some(expression_instance) = def_decision.decision_logic().as_ref() {
        let (evaluator, _) =
            build_expression_instance_evaluator(&scope, expression_instance, model_builder)?;
        evaluator
    } else {
    ...
    for knowledge_requirement in def_decision.knowledge_requirements() {
        ...
        required_knowledge_references.push((
            required_knowledge.import_name().cloned(),
            required_knowledge.into(),
        ));
    }
    ...
    let decision_evaluator = Box::new(
        move |global_context: &FeelContext,
              input_data_ctx: &FeelContext,
              model_evaluator: &ModelEvaluator,
              output_data_ctx: &mut FeelContext| {
            let business_knowledge_model_evaluator =
                model_evaluator.business_knowledge_model_evaluator();
            ...
            required_knowledge_references
                .iter()
                .for_each(|(import_name, def_key)| {
                    if let Some(import_name_parent) = import_name.clone() {
                        if let Some(name) = business_knowledge_model_evaluator.evaluate(
                            def_key,
                            global_context,
                            &input_data,
                            model_evaluator,
                            &mut requirements_ctx,
                        ) {
            ...
            required_knowledge_references
                ...
                .for_each(|(import_name, def_key)| {
                    if let Some(import_name_parent) = import_name.clone() {
                        if let Some(name) = decision_service_evaluator.evaluate_fd(
                            def_key,
                            &input_data,
                            &mut requirements_ctx,
                        ) {
                        ...
                        decision_service_evaluator.evaluate_fd(
                            def_key,
                            &input_data,
                            &mut requirements_ctx,
                        );
                ...
                .for_each(|(import_name, def_key)| {
                    if let Some(import_name_parent) = import_name.clone() {
                        ...
                        if import_input_data.is_context(&import_name_parent) {
                            if let Some(Value::Context(ctx)) =
                                import_input_data.remove_entry(&import_name_parent)
                            {
                                import_input_data.zip(&ctx);
                            }
                            input_data.remove_entry(&import_name_parent);
                        }
                        if let Some(name) = decision_evaluator.evaluate(
                            def_key,
                            global_context,
                            &import_input_data,
                            model_evaluator,
            ...
            required_input_data_references
                .iter()
                .for_each(|input_data_id| {
                    if let Some((name, value)) = input_data_evaluator.evaluate(
                        input_data_id,
                        &input_data,
