...
impl EvaluatedDecisionTable {
    ...
    fn get_matching_rules(&self) -> Vec<&EvaluatedRule> {
        self.evaluated_rules
            .iter()
            .filter(|evaluated_rule| evaluated_rule.matches)
            .collect()
    }
    /// Returns all matching rules in decreasing order of priority.
    fn get_matching_rules_prioritized(&self) -> Vec<&EvaluatedRule> {
        let mut rules: Vec<&EvaluatedRule> =
            self.evaluated_rules.iter().filter(|v| v.matches).collect();
        let compare = |x: &&EvaluatedRule, y: &&EvaluatedRule| {
            for (v1, v2) in x
                .output_entry_values
                .iter()
                .zip(y.output_entry_values.iter())
            {
...
fn parse_decision_table(
    ...
    for input_clause in decision_table.input_clauses() {
        let input_expression =
            dmntk_feel_parser::parse_expression(scope, &input_clause.input_expression, false)?;
    ...
    for rule in decision_table.rules() {
        ...
        {
            let input_entry_node =
                dmntk_feel_parser::parse_unary_tests(scope, &rule.input_entries[i].text, false)?;
            if let Some(input_values_node) = input_values {
                let left = AstNode::In(
                    Box::new(input_expression.clone()),
                    Box::new(input_values_node.clone()),
                );
                let right = AstNode::In(
                    Box::new(input_expression.clone()),
                    Box::new(input_entry_node),
                );
                let node = AstNode::And(Box::new(left), Box::new(right));
                input_entries_evaluators.push(dmntk_feel_evaluator::prepare(
        ...
        for (i, output_values) in output_values_nodes.iter().enumerate() {
            let output_entry_node =
                dmntk_feel_parser::parse_expression(scope, &rule.output_entries[i].text, false)?;
            if let Some(output_value_node) = output_values {
                let node = AstNode::Out(
                    Box::new(output_entry_node),
                    Box::new(output_value_node.clone()),
                );
                output_entries_evaluators.push(dmntk_feel_evaluator::prepare(
                    &BuildContext::default(),
                    &node,
                )?);
            } else {
    ...
    for opt_node in output_values_nodes {
        if let Some(node) = opt_node {
            output_values_evaluators.push(Some(dmntk_feel_evaluator::prepare(
                &BuildContext::default(),
                &node,
            )?));
        } else {
    ...
    for opt_node in default_output_values_nodes {
        if let Some(node) = opt_node {
            default_output_values_evaluators.push(Some(dmntk_feel_evaluator::prepare(
                &BuildContext::default(),
                &node,
            )?));
        } else {
...
///
fn evaluate_parsed_decision_table(
    scope: &FeelScope,
    parsed_decision_table: &ParsedDecisionTable,
) -> EvaluatedDecisionTable {
    // evaluate only non-empty output values
    let mut output_values = vec![];
    for evaluator in parsed_decision_table
        .output_values_evaluators
        .iter()
        .flatten()
    {
...
pub fn build_decision_table_evaluator(
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        let evaluated_decision_table =
            evaluate_parsed_decision_table(scope, &parsed_decision_table);
        match hit_policy {
            ...
            HitPolicy::Collect(aggregator) => match aggregator {
                BuiltinAggregator::List => {
                    evaluated_decision_table.evaluate_hit_policy_collect_list()
                }
                BuiltinAggregator::Count => {
                    evaluated_decision_table.evaluate_hit_policy_collect_count()
                }
                BuiltinAggregator::Sum => {
                    evaluated_decision_table.evaluate_hit_policy_collect_sum()
                }
                BuiltinAggregator::Min => {
                    evaluated_decision_table.evaluate_hit_policy_collect_min()
                }
                BuiltinAggregator::Max => {
                    evaluated_decision_table.evaluate_hit_policy_collect_max()
                }
            },
        }
    }))
