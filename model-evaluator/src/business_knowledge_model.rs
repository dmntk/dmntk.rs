...
///
type BusinessKnowledgeModelEvaluatorFn = Box<
...
impl BusinessKnowledgeModelEvaluator {
    ...
    pub fn new(definitions: &DefDefinitions, model_builder: &ModelBuilder) -> Result<Self> {
        ...
        for business_knowledge_model in definitions.business_knowledge_models() {
            ...
            let evaluator = build_bkm_evaluator(
                definitions,
            ...
            model_builder.add_bkm_invocable(
                namespace.to_string(),
                name,
                def_key,
    ...
    pub fn evaluate(
        ...
        self.evaluators.get(def_key).map(|evaluator_entry| {
            evaluator_entry(global_context, input_data, model_evaluator, output_data)
        })
    }
}
...
fn build_bkm_evaluator(
    ...
    let output_variable_type = item_definition_type_evaluator
        .information_item_type(
            business_knowledge_model.variable().namespace(),
            business_knowledge_model.variable().type_ref(),
        )
    ...
    if let Some(expression_instance) = function_definition.body() {
        ...
        let output_variable_name = business_knowledge_model.variable().name().clone();
        Ok(Box::new(
            move |_: &FeelContext, _: &FeelContext, _: &ModelEvaluator, _: &mut FeelContext| {
...
fn build_bkm_context_evaluator(
    ...
    build_bkm_evaluator_from_function_definition(
        output_variable_name,
        function,
        knowledge_requirements,
    )
    ...
    Ok(Box::new(
        move |global_context: &FeelContext,
            ...
            knowledge_requirements.iter().for_each(|def_key| {
                ...
                decision_service_evaluator.evaluate(
                    def_key,
                    global_context,
                    input_data,
                    model_evaluator,
