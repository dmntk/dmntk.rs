...
///
type DecisionServiceEvaluatorFn = Box<
    dyn Fn(&FeelContext, &FeelContext, &ModelEvaluator, &mut FeelContext) -> Name + Send + Sync,
>;

///
type DecisionServiceEvaluatorEntry = (
    Variable,
    Vec<(Name, FeelType)>,
    DecisionServiceEvaluatorFn,
    Option<Evaluator>,
);
    ...
    pub fn build_function_definitions(&self, model_evaluator: &Arc<ModelEvaluator>) {
        if let Ok(mut evaluators) = self.evaluators.write() {
            ...
            for decision_service_id in identifiers {
                ...
                evaluators
                    .entry(decision_service_id.clone())
                    .and_modify(|entry| {
                        ...
                        let body_evaluator = Box::new(move |scope: &FeelScope| {
                            ...
                            let opt_out_variable_name = decision_service_evaluator.evaluate(
                                &decision_service_id,
                                &global_context,
                                &input_data,
                                &evaluator,
                            ...
                            if let Some(out_variable_name) = opt_out_variable_name {
                                if let Some(result_value) =
                                    output_data.get_entry(&out_variable_name)
                                {
                        ...
                        let function_body =
                            dmntk_feel::FunctionBody::DecisionService(Arc::new(body_evaluator));
                        ...
                        let decision_service_as_function_definition_evaluator =
                            Box::new(move |_: &FeelScope| function_definition.clone());
...
fn build_decision_service_evaluator(
    decision_service: &DefDecisionService,
    ...
    let input_data_references: Vec<DefKey> = decision_service
        .input_data()
        .iter()
        .map(|href| href.into())
        .collect();
    ...
    let input_decisions: Vec<DefKey> = decision_service
        .input_decisions()
        .iter()
        .map(|href| href.into())
        .collect();
    ...
    let encapsulated_decisions: Vec<DefKey> = decision_service
    ...
    let output_decisions: Vec<DefKey> = decision_service
        .output_decisions()
        .iter()
        .map(|href| href.into())
        .collect();
    ...
    for input_data_id in &input_data_references {
        if let Some(input_data_variable) = input_data_evaluator.get_variable(input_data_id) {
            ...
            let parameter_type =
                input_data_variable.resolve_feel_type(item_definition_type_evaluator);
    ...
    for def_key in &input_decisions {
        if let Some(decision_output_variable) = decision_evaluator.get_variable(def_key) {
            ...
            let parameter_type =
                decision_output_variable.resolve_feel_type(item_definition_type_evaluator);
            formal_parameters.push((parameter_name, parameter_type));
    ...
    let decision_service_evaluator = Box::new(
        move |global_context: &FeelContext,
            ...
            for evaluator in &input_decision_results_evaluators {
                let (name, value) =
                    evaluator(&input_decision_results_value, item_definition_evaluator);
            ...
            output_decisions.iter().for_each(|def_key| {
                if let Some(output_name) = decision_evaluator.evaluate(
                    def_key,
                    global_context,
                    &evaluated_input_data,
                    model_evaluator,
