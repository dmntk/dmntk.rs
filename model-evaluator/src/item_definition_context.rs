...
type ItemDefinitionContextEvaluatorFn =
    Box<dyn Fn(&Name, &mut FeelContext, &ItemDefinitionContextEvaluator) -> FeelType + Send + Sync>;
...
///
fn item_definition_context_evaluator(
    item_definition: &DefItemDefinition,
) -> Result<ItemDefinitionContextEvaluatorFn> {
    match item_definition.item_definition_type()? {
        ...
        ItemDefinitionType::ReferencedType(namespace, ref_type) => {
            referenced_type_context_evaluator(DefKey::new(&namespace, &ref_type))
        }
        ...
        ItemDefinitionType::CollectionOfSimpleType(feel_type) => {
            collection_of_simple_type_context_evaluator(feel_type)
        }
        ItemDefinitionType::CollectionOfReferencedType(namespace, ref_type) => {
            collection_of_referenced_type_context_evaluator(DefKey::new(&namespace, &ref_type))
        }
        ItemDefinitionType::CollectionOfComponentType => {
            collection_of_component_type_context_evaluator(item_definition)
        }
...
fn simple_type_context_evaluator(feel_type: FeelType) -> Result<ItemDefinitionContextEvaluatorFn> {
    if matches!(
        ...
        Ok(Box::new(
            move |name: &Name, ctx: &mut FeelContext, _: &ItemDefinitionContextEvaluator| {
...
///
fn component_type_context_evaluator(
    item_definition: &DefItemDefinition,
) -> Result<ItemDefinitionContextEvaluatorFn> {
...
///
fn collection_of_simple_type_context_evaluator(
    feel_type: FeelType,
) -> Result<ItemDefinitionContextEvaluatorFn> {
    if matches!(
        feel_type,
...
///
fn collection_of_referenced_type_context_evaluator(
    def_key: DefKey,
) -> Result<ItemDefinitionContextEvaluatorFn> {
    Ok(Box::new(
        move |name: &Name, ctx: &mut FeelContext, evaluator: &ItemDefinitionContextEvaluator| {
...
///
fn collection_of_component_type_context_evaluator(
    item_definition: &DefItemDefinition,
) -> Result<ItemDefinitionContextEvaluatorFn> {
    let mut context_evaluators = vec![];
    for component_item_definition in item_definition.item_components() {
...
///
fn function_type_context_evaluator(
    _item_definition: &DefItemDefinition,
) -> Result<ItemDefinitionContextEvaluatorFn> {
    Ok(Box::new(
        move |_name: &Name, _ctx: &mut FeelContext, _: &ItemDefinitionContextEvaluator| {
            FeelType::Any
        },
    ))
...
mod tests {
    ...
    fn simple_type_string() {
        ...
        let actual_type = evaluator.eval(
            &DefKey::new(NAMESPACE, "tCustomerName"),
            &variable_name,
            &mut ctx,
        );
    ...
    */
    ...
    fn collection_of_simple_type_string() {
        ...
        expected_context.set_entry(
            &variable_name,
            Value::List(vec![Value::FeelType(FeelType::String)]),
        );
        let actual_type =
            evaluator.eval(&DefKey::new(NAMESPACE, "tItems"), &variable_name, &mut ctx);
        assert_eq!(expected_type, actual_type);
        assert_eq!(expected_context, ctx);
