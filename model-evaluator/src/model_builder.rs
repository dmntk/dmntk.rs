...
impl ModelBuilder {
  ...
  pub fn add_decision_invocable(&self, namespace: String, name: String, def_key: DefKey) {
    self.invocables.borrow_mut().add_decision(namespace, name, def_key);
  }

  pub fn add_bkm_invocable(&self, namespace: String, name: String, def_key: DefKey, output_variable_name: Name) {
    self.invocables.borrow_mut().add_bkm(namespace, name, def_key, output_variable_name);
  }

  pub fn add_decision_service_invocable(&self, namespace: String, name: String, def_key: DefKey) {
    self.invocables.borrow_mut().add_decision_service(namespace, name, def_key);
  }
}
