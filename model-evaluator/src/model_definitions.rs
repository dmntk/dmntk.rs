...
use dmntk_common_1::{HRef, Result};
...
impl DefInformationItem {
    ...
    pub fn new(information_item: &InformationItem, imports: &[DefImport]) -> Self {
        ...
        if qname.len() == 2 {
            ...
            let namespace = get_import_namespace(import_name, imports)
                .unwrap_or(information_item.namespace().to_string());
...
impl DefItemDefinition {
    pub fn new(item_definition: &ItemDefinition) -> Self {
        Self {
            ...
            item_components: item_definition
                .item_components()
                .iter()
                .map(DefItemDefinition::new)
                .collect(),
...
impl DefItemDefinition {
    ...
    pub fn item_definition_type(&self) -> Result<ItemDefinitionType> {
        let simple_type_ref = if let Some(type_ref) = self.type_ref() {
        ...
        match condition {
            (_, true, false, false, false) => Ok(ItemDefinitionType::SimpleType(
                simple_type_ref.unwrap().clone(),
            )),
            (true, false, false, false, false) => Ok(ItemDefinitionType::ReferencedType(
                self.namespace.clone(),
                self.type_ref().as_ref().unwrap().clone(),
            )),
            (false, false, true, false, false) => Ok(ItemDefinitionType::ComponentType),
            (_, true, false, true, false) => Ok(ItemDefinitionType::CollectionOfSimpleType(
                simple_type_ref.unwrap().clone(),
            )),
            (false, false, true, true, false) => Ok(ItemDefinitionType::CollectionOfComponentType),
            (true, false, false, true, false) => {
                Ok(ItemDefinitionType::CollectionOfReferencedType(
                    self.namespace.clone(),
                    self.type_ref().as_ref().unwrap().clone(),
                ))
            }
    ...
    pub fn new(business_knowledge_model: &BusinessKnowledgeModel, imports: &[DefImport]) -> Self {
        Self {
            ...
            knowledge_requirements: business_knowledge_model
                ...
                .iter()
                .map(|knowledge_requirements| {
                    DefKnowledgeRequirement::new(knowledge_requirements, imports)
                })
                .collect(),
        }
    }
...
impl DefKnowledgeRequirement {
    pub fn new(knowledge_requirement: &KnowledgeRequirement, imports: &[DefImport]) -> Self {
        Self {
            required_knowledge: DefHRef::new(
                knowledge_requirement.namespace(),
                knowledge_requirement.required_knowledge(),
                imports,
            ),
        }
    }
    ...
    pub fn new(decision: &Decision, imports: &[DefImport]) -> Self {
        Self {
            ...
            information_requirements: decision
                ...
                .iter()
                .map(|knowledge_requirements| {
                    DefInformationRequirement::new(knowledge_requirements, imports)
                })
                .collect(),
...
impl DefDecisionService {
    ...
    pub fn new(decision_service: &DecisionService, imports: &[DefImport]) -> Self {
        ...
        Self {
            ...
            input_decisions: decision_service
                .input_decisions()
                .iter()
...
impl DefDefinitions {
    ...
    pub fn add_model(&mut self, definitions: &Definitions) {
        self.imports
            .append(&mut definitions.imports().iter().map(DefImport::new).collect());
        self.item_definitions.append(
            &mut definitions
                .item_definitions()
                .iter()
                .map(DefItemDefinition::new)
            ...
            match drg_element {
                DrgElement::InputData(inner) => {
                    self.input_data.insert(
                        DefKey::new(inner.namespace(), inner.id()),
                        DefInputData::new(inner, &self.imports),
                    );
                }
                DrgElement::BusinessKnowledgeModel(inner) => {
                    self.business_knowledge_models.insert(
                        DefKey::new(inner.namespace(), inner.id()),
                        DefBusinessKnowledgeModel::new(inner, &self.imports),
                    );
                }
                DrgElement::Decision(inner) => {
                    self.decisions.insert(
                        DefKey::new(inner.namespace(), inner.id()),
                        DefDecision::new(inner, &self.imports),
                    );
                }
                DrgElement::DecisionService(inner) => {
                    self.decision_services.insert(
                        DefKey::new(inner.namespace(), inner.id()),
                        DefDecisionService::new(inner, &self.imports),
                    );
                }
                _ => {}
            }
    ...
    pub fn business_knowledge_model_by_key(
        &self,
        namespace: &str,
        id: &str,
    ) -> Option<&DefBusinessKnowledgeModel> {
        self.business_knowledge_models
            .get(&DefKey::new(namespace, id))
    }
...
fn get_import_name(namespace: &str, imports: &[DefImport]) -> Option<Name> {
    imports
        .iter()
        .filter_map(|def_import| {
            if def_import.namespace == namespace {
                Some(def_import.name.clone())
            } else {
                None
            }
        })
...
fn get_import_namespace(name: &Name, imports: &[DefImport]) -> Option<String> {
    imports
        .iter()
        .filter_map(|def_import| {
            if &def_import.name == name {
                Some(def_import.namespace.clone())
            } else {
                None
            }
        })
