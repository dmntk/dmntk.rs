...
use dmntk_feel_evaluator::{
    evaluate, evaluate_context, evaluate_context_node, evaluate_max, evaluate_min, evaluate_sum,
...
fn _0002() {
    ...
    assert_eq!(
        "{alpha: true}",
        evaluate_context_node(&scope, &node).unwrap().to_string()
    );
}
...
#[test]
fn _0005() {
    let value = evaluate_sum(vec![
        Value::Number(FeelNumber::new(123, 2)),
        Value::Number(FeelNumber::new(177, 2)),
    ]);
    assert_eq!("3", value.to_string());
}
