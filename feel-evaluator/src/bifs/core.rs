...
use dmntk_feel::{
    value_null, value_number, value_string, FeelNumber, FeelScope, Name, ToFeelString,
};
use dmntk_feel_temporal::{
    DayOfWeek, DayOfYear, FeelDate, FeelDateTime, FeelDaysAndTimeDuration, FeelTime,
    FeelYearsAndMonthsDuration, MonthOfYear, WeekOfYear,
};
...
pub fn after(value1: &Value, value2: &Value) -> Value {
    match value1 {
        ...
        Value::Range(range1_start, closed1_start, _, _) => match range1_start.borrow() {
            Value::Number(point1) => match value2 {
                Value::Number(point2) => {
                    return Value::Boolean(point1 > point2 || (point1 == point2 && !*closed1_start))
                }
                Value::Range(_, _, range2_end, closed2_end) => {
                    if let Value::Number(point2) = range2_end.borrow() {
                        return Value::Boolean(
                            point1 > point2
                                || (point1 == point2 && (!*closed1_start || !*closed2_end)),
                        );
                    }
                }
                _ => {}
    ...
    match value1 {
        Value::Number(point1) => match value2 {
            ...
            Value::Range(range_start2, closed_start2, _, _) => {
                if let Value::Number(start2) = range_start2.borrow() {
                    return Value::Boolean(
                        point1 < start2 || (point1 == start2 && !*closed_start2),
                    );
                }
            }
            _ => {}
        ...
        Value::Range(_, _, range_end1, closed_end1) => match range_end1.borrow() {
            Value::Number(end1) => match value2 {
                Value::Number(point2) => {
                    return Value::Boolean(end1 < point2 || (!*closed_end1 && end1 == point2))
                }
                Value::Range(range_start2, closed_start2, _, _) => {
                    if let Value::Number(start2) = range_start2.borrow() {
                        return Value::Boolean(
                            end1 < start2 || (end1 == start2 && (!*closed_end1 || !*closed_start2)),
                        );
                    }
                }
                _ => {}
...
pub fn coincides(value1: &Value, value2: &Value) -> Value {
    match value1 {
        ...
        Value::Range(range1_start, closed1_start, range1_end, closed1_end) => {
            match (range1_start.borrow(), range1_end.borrow()) {
                (Value::Number(point1_start), Value::Number(point1_end)) => {
                    if let Value::Range(range2_start, closed2_start, range2_end, closed2_end) =
                        value2
                    {
                        if let (Value::Number(point2_start), Value::Number(point2_end)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(
                                point1_start == point2_start
                                    && point1_end == point2_end
                                    && closed1_start == closed2_start
                                    && closed1_end == closed2_end,
                            );
                        }
                    }
                }
                (Value::Date(point1_start), Value::Date(point1_end)) => {
                    if let Value::Range(range2_start, closed2_start, range2_end, closed2_end) =
                        value2
                    {
                        if let (Value::Date(point2_start), Value::Date(point2_end)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(
                                point1_start == point2_start
                ...
                (Value::Time(point1_start), Value::Time(point1_end)) => {
                    if let Value::Range(range2_start, closed2_start, range2_end, closed2_end) =
                        value2
                    {
                        if let (Value::Time(point2_start), Value::Time(point2_end)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(
                                point1_start == point2_start
                ...
                (Value::DateTime(point1_start), Value::DateTime(point1_end)) => {
                    if let Value::Range(range2_start, closed2_start, range2_end, closed2_end) =
                        value2
                    {
                        if let (Value::DateTime(point2_start), Value::DateTime(point2_end)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(
                                point1_start == point2_start
                ...
                (
                    Value::DaysAndTimeDuration(point1_start),
                    Value::DaysAndTimeDuration(point1_end),
                ) => {
                    if let Value::Range(range2_start, closed2_start, range2_end, closed2_end) =
                        value2
                    {
                        if let (
                            Value::DaysAndTimeDuration(point2_start),
                            Value::DaysAndTimeDuration(point2_end),
                        ) = (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(
                                point1_start == point2_start
                ...
                (
                    Value::YearsAndMonthsDuration(point1_start),
                    Value::YearsAndMonthsDuration(point1_end),
                ) => {
                    if let Value::Range(range2_start, closed2_start, range2_end, closed2_end) =
                        value2
                    {
                        if let (
                            Value::YearsAndMonthsDuration(point2_start),
                            Value::YearsAndMonthsDuration(point2_end),
                        ) = (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(
                                point1_start == point2_start
        ...
        if let Value::Number(month) = month_value {
            if let Value::Number(day) = day_value {
                if let Ok(date) = FeelDate::try_from((*year, *month, *day)) {
                    ...
                    value_null!(
                        "[core::date_3] invalid date y={} m={} d={}",
                        year,
                        month,
                        day
                    )
...
pub fn date_and_time_1(value: &Value) -> Value {
    if let Value::String(text) = value {
        ...
        value_null!(
            "[core::date and time] invalid date or date and time '{}'",
            text
        )
    } else {
...
pub fn decimal(number_value: &Value, scale_value: &Value) -> Value {
    if let Value::Number(number) = number_value {
        if let Value::Number(scale) = scale_value {
            ...
            value_null!(
                "[core::decimal] scale value is not a number: {}",
                scale_value
            )
        }
    } else {
        value_null!(
            "[core::decimal] number value is not a number: {}",
            number_value
        )
    }
}
...
macro_rules! during_rr {
    ($r1s:expr, $c1s:expr, $r1e:expr, $c1e:expr, $r2s:expr, $c2s:expr, $r2e:expr, $c2e:expr) => {
        ($r1s > $r2s || ($r1s == $r2s && (*$c1s == *$c2s || *$c2s)))
            && ($r1e < $r2e || ($r1e == $r2e && (*$c1e == *$c2e || *$c2e)))
    };
}
    ...
    match value1 {
        Value::Number(point) => {
            if let Value::Range(range_start, closed_start, range_end, closed_end) = value2 {
                if let (Value::Number(point1), Value::Number(point2)) =
                    (range_start.borrow(), range_end.borrow())
                {
                    return Value::Boolean(
                        (point > point1 || (point == point1 && *closed_start))
                            && (point < point2 || (point == point2 && *closed_end)),
                    );
                }
            }
        }
        Value::Date(point) => {
            if let Value::Range(range_start, closed_start, range_end, closed_end) = value2 {
                if let (Value::Date(point1), Value::Date(point2)) =
                    (range_start.borrow(), range_end.borrow())
                {
                    return Value::Boolean(
                        (point > point1 || (point == point1 && *closed_start))
        ...
        Value::Time(point) => {
            if let Value::Range(range_start, closed_start, range_end, closed_end) = value2 {
                if let (Value::Time(point1), Value::Time(point2)) =
                    (range_start.borrow(), range_end.borrow())
                {
                    return Value::Boolean(
                        (point > point1 || (point == point1 && *closed_start))
        ...
        Value::DateTime(point) => {
            if let Value::Range(range_start, closed_start, range_end, closed_end) = value2 {
                if let (Value::DateTime(point1), Value::DateTime(point2)) =
                    (range_start.borrow(), range_end.borrow())
                {
                    return Value::Boolean(
                        (point > point1 || (point == point1 && *closed_start))
        ...
        Value::DaysAndTimeDuration(point) => {
            if let Value::Range(range_start, closed_start, range_end, closed_end) = value2 {
                if let (Value::DaysAndTimeDuration(point1), Value::DaysAndTimeDuration(point2)) =
                    (range_start.borrow(), range_end.borrow())
                {
                    return Value::Boolean(
                        (point > point1 || (point == point1 && *closed_start))
        ...
        Value::Range(range1_start, c1s, range1_end, c1e) => {
            match (range1_start.borrow(), range1_end.borrow()) {
                (Value::Number(r1s), Value::Number(r1e)) => {
                    if let Value::Range(range2_start, c2s, range2_end, c2e) = value2 {
                        if let (Value::Number(r2s), Value::Number(r2e)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(during_rr!(
                                r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e
                            ));
                        }
                    }
                }
                (Value::Date(r1s), Value::Date(r1e)) => {
                    if let Value::Range(range2_start, c2s, range2_end, c2e) = value2 {
                        if let (Value::Date(r2s), Value::Date(r2e)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(during_rr!(
                                r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e
                ...
                (Value::Time(r1s), Value::Time(r1e)) => {
                    if let Value::Range(range2_start, c2s, range2_end, c2e) = value2 {
                        if let (Value::Time(r2s), Value::Time(r2e)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(during_rr!(
                                r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e
                ...
                (Value::DateTime(r1s), Value::DateTime(r1e)) => {
                    if let Value::Range(range2_start, c2s, range2_end, c2e) = value2 {
                        if let (Value::DateTime(r2s), Value::DateTime(r2e)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(during_rr!(
                                r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e
                ...
                (Value::DaysAndTimeDuration(r1s), Value::DaysAndTimeDuration(r1e)) => {
                    if let Value::Range(range2_start, c2s, range2_end, c2e) = value2 {
                        if let (Value::DaysAndTimeDuration(r2s), Value::DaysAndTimeDuration(r2e)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(during_rr!(
                                r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e
                ...
                (Value::YearsAndMonthsDuration(r1s), Value::YearsAndMonthsDuration(r1e)) => {
                    if let Value::Range(range2_start, c2s, range2_end, c2e) = value2 {
                        if let (
                            Value::YearsAndMonthsDuration(r2s),
                            Value::YearsAndMonthsDuration(r2e),
                        ) = (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(during_rr!(
                                r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e
                ...
                match range1_end.borrow() {
                    Value::Number(r1_end) => {
                        if let Value::Number(r2_end) = range2_end.borrow() {
                            return Value::Boolean(
                                *closed1_end == *closed2_end && r1_end == r2_end,
                            );
                        }
                    }
                    Value::Date(r1_end) => {
        ...
        match range1_end.borrow() {
            Value::Number(point1) => match value2 {
                ...
                Value::Range(_, _, range2_end, closed2_end) => match range2_end.borrow() {
                    Value::Number(point2) => {
                        Value::Boolean(*closed1_end == *closed2_end && point1 == point2)
                    }
                    _ => invalid_argument_type!("finished by", "range<number>", value2.type_of()),
                },
                _ => invalid_argument_type!(
                    "finished by",
                    "number or range<number>",
                    value2.type_of()
                ),
            },
            Value::Date(point1) => match value2 {
            ...
            Value::DateTime(point1) => match value2 {
                ...
                _ => invalid_argument_type!(
                    "finished by",
                    "date and time or range<date and time>",
                    value2.type_of()
                ),
            },
            Value::DaysAndTimeDuration(point1) => match value2 {
                Value::DaysAndTimeDuration(point2) => {
                    Value::Boolean(*closed1_end && point1 == point2)
                }
                Value::Range(_, _, range2_end, closed2_end) => match range2_end.borrow() {
                    Value::DaysAndTimeDuration(point2) => {
                        Value::Boolean(*closed1_end == *closed2_end && point1 == point2)
                    ...
                    _ => invalid_argument_type!(
                        "finished by",
                        "range<days and time duration>",
                        value2.type_of()
                    ),
                },
                _ => invalid_argument_type!(
                    "finished by",
                    "days and time duration or range<days and time duration>",
                    value2.type_of()
                ),
            },
            Value::YearsAndMonthsDuration(point1) => match value2 {
                ...
                Value::Range(_, _, range2_end, closed2_end) => match range2_end.borrow() {
                    ...
                    _ => invalid_argument_type!(
                        "finished by",
                        "range<years and months duration>",
                        value2.type_of()
                    ),
                },
                _ => invalid_argument_type!(
                    "finished by",
                    "years and months duration or range<years and months duration>",
                    value2.type_of()
                ),
            },
...
macro_rules! includes_rr {
    ($r1s:expr, $c1s:expr, $r1e:expr, $c1e:expr, $r2s:expr, $c2s:expr, $r2e:expr, $c2e:expr) => {
        ($r2s > $r1s || ((*$c1s == *$c2s || *$c1s) && $r2s == $r1s))
            && ($r2e < $r1e || ((*$c1e == *$c2e || *$c1e) && $r2e == $r1e))
    };
}
    ...
    if let Value::Range(range1_start, c1s, range1_end, c1e) = value1 {
        match (range1_start.borrow(), range1_end.borrow()) {
            (Value::Number(r1s), Value::Number(r1e)) => match value2 {
                Value::Range(range2_start, c2s, range2_end, c2e) => {
                    if let (Value::Number(r2s), Value::Number(r2e)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(includes_rr!(
                            r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e
                        ));
                    }
            ...
            (Value::DaysAndTimeDuration(r1s), Value::DaysAndTimeDuration(r1e)) => match value2 {
                ...
                Value::DaysAndTimeDuration(point2) => {
                    Value::Boolean(includes_rp!(r1s, c1s, r1e, c1e, point2))
                }
                _ => invalid_argument_type!(
                    "includes",
                    "days and time duration or range<days and time duration>",
...
pub fn matches(
    ...
    pattern_string_value: &Value,
    flags_string_value: &Value,
) -> Value {
    if let Value::String(input_string) = input_string_value {
        ...
        match (range1_start.borrow(), range1_end.borrow()) {
            (Value::Number(_), Value::Number(point1_end)) => {
                if let Value::Range(range2_start, closed2_start, range2_end, _) = value2 {
                    if let (Value::Number(point2_start), Value::Number(_)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_end == point2_start && closed1_end == closed2_start,
                        );
                    }
                }
            }
            (Value::Date(_), Value::Date(point1_end)) => {
                if let Value::Range(range2_start, closed2_start, range2_end, _) = value2 {
                    if let (Value::Date(point2_start), Value::Date(_)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_end == point2_start && closed1_end == closed2_start,
            ...
            (Value::Time(_), Value::Time(point1_end)) => {
                if let Value::Range(range2_start, closed2_start, range2_end, _) = value2 {
                    if let (Value::Time(point2_start), Value::Time(_)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_end == point2_start && closed1_end == closed2_start,
            ...
            (Value::DateTime(_), Value::DateTime(point1_end)) => {
                if let Value::Range(range2_start, closed2_start, range2_end, _) = value2 {
                    if let (Value::DateTime(point2_start), Value::DateTime(_)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_end == point2_start && closed1_end == closed2_start,
        ...
        if let Value::Range(range2_start, _, range2_end, closed2_end) = value2 {
            return match (range1_start.borrow(), range1_end.borrow()) {
                (Value::Number(point1_start), Value::Number(_)) => {
                    if let (Value::Number(_), Value::Number(point2_end)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_start == point2_end && closed1_start == closed2_end,
                        );
                    }
                    invalid_argument_type!("meets", "range<number>", value2.type_of())
                }
                (Value::Date(point1_start), Value::Date(_)) => {
                    if let (Value::Date(_), Value::Date(point2_end)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_start == point2_end && closed1_start == closed2_end,
                ...
                (Value::Time(point1_start), Value::Time(_)) => {
                    if let (Value::Time(_), Value::Time(point2_end)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_start == point2_end && closed1_start == closed2_end,
                ...
                (Value::DateTime(point1_start), Value::DateTime(_)) => {
                    if let (Value::DateTime(_), Value::DateTime(point2_end)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_start == point2_end && closed1_start == closed2_end,
                ...
                (Value::DaysAndTimeDuration(point1_start), Value::DaysAndTimeDuration(_)) => {
                    if let (Value::DaysAndTimeDuration(_), Value::DaysAndTimeDuration(point2_end)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        return Value::Boolean(
                            point1_start == point2_end && closed1_start == closed2_end,
        ...
        Value::String(value) => {
            ...
            let grouping_sep = match grouping_separator {
                Value::String(s) => match s.as_str() {
                    " " | "." | "," => Some((*s).clone()),
                    _ => return value_null!(
                        "[core::number] grouping separator must be space, period, comma or null"
                    ),
                },
                Value::Null(_) => None,
                _ => {
            ...
            let decimal_sep = match decimal_separator {
                Value::String(s) => match s.as_str() {
                    ...
                    _ => {
                        return value_null!(
                            "[core::number] decimal separator must be period, comma or null"
                        )
                    }
                },
                Value::Null(_) => None,
...
macro_rules! overlaps_rr {
    ($r1s:expr, $c1s:expr, $r1e:expr, $c1e:expr, $r2s:expr, $c2s:expr, $r2e:expr, $c2e:expr) => {
        ($r1e >= $r2s && ($r1e != $r2s || (*$c1e == *$c2s && (*$c1e || *$c2s))))
            && ($r2e >= $r1s && ($r2e != $r1s || (*$c2e == *$c1s && (*$c2e || *$c1s))))
    };
}
...
pub fn overlaps(value1: &Value, value2: &Value) -> Value {
    if let Value::Range(range1_start, c1s, range1_end, c1e) = value1 {
        if let Value::Range(range2_start, c2s, range2_end, c2e) = value2 {
            match (range1_start.borrow(), range1_end.borrow()) {
                (Value::Number(r1s), Value::Number(r1e)) => {
                    return if let (Value::Number(r2s), Value::Number(r2e)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                ...
                (Value::Date(r1s), Value::Date(r1e)) => {
                    return if let (Value::Date(r2s), Value::Date(r2e)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        Value::Boolean(overlaps_rr!(r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e))
                    } else {
                ...
                (Value::Time(r1s), Value::Time(r1e)) => {
                    return if let (Value::Time(r2s), Value::Time(r2e)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        Value::Boolean(overlaps_rr!(r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e))
                    } else {
                ...
                (Value::DateTime(r1s), Value::DateTime(r1e)) => {
                    return if let (Value::DateTime(r2s), Value::DateTime(r2e)) =
                        (range2_start.borrow(), range2_end.borrow())
                    {
                        Value::Boolean(overlaps_rr!(r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e))
                    } else {
                ...
                (Value::DaysAndTimeDuration(r1s), Value::DaysAndTimeDuration(r1e)) => {
                    return if let (
                        Value::DaysAndTimeDuration(r2s),
                        Value::DaysAndTimeDuration(r2e),
                    ) = (range2_start.borrow(), range2_end.borrow())
                    {
                        Value::Boolean(overlaps_rr!(r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e))
                    } else {
...
pub fn replace(
    input_string_value: &Value,
    pattern_string_value: &Value,
    replacement_string_value: &Value,
    flags_string_value: &Value,
) -> Value {
    if let Value::String(input_string) = input_string_value {
        if let Value::String(pattern_string) = pattern_string_value {
            if let Value::String(replacement_string) = replacement_string_value {
                let repl = RG_REPLACE_NUM
                    .replace_all(replacement_string.as_str(), "$${${1}}")
                    .to_string();
                // check and use flags
                if let Value::String(flags_string) = flags_string_value {
                    ...
                    if flags.is_empty() {
                        if let Ok(re) = Regex::new(&patt) {
                            let result = re
                                .replace_all(input_string.as_str(), repl.as_str())
                                .to_string();
...
pub fn sort(list: &Value, ordering_function: &Value) -> Value {
    if let Value::List(items) = list {
        if let Value::FunctionDefinition(parameters, body, false, _, closure_ctx, _) =
            ordering_function
        {
            if parameters.len() == 2 {
    ...
    if let Value::Range(range1_start, c1s, _, _) = value1 {
        match range1_start.borrow() {
            ...
            Value::DaysAndTimeDuration(r1s) => match value2 {
                ...
                Value::DaysAndTimeDuration(point2) => {
                    Value::Boolean(started_by_rp!(r1s, c1s, point2))
                }
                _ => invalid_argument_type!(
                    "started by",
                    "days and time duration or range<days and time duration>",
                ...
                (Value::Number(r1s), Value::Number(r1e)) => {
                    if let Value::Range(range2_start, c2s, range2_end, c2e) = value2 {
                        if let (Value::Number(r2s), Value::Number(r2e)) =
                            (range2_start.borrow(), range2_end.borrow())
                        {
                            return Value::Boolean(starts_rr!(
                                r1s, c1s, r1e, c1e, r2s, c2s, r2e, c2e
                            ));
                        }
                    }
...
pub fn stddev(values: &[Value]) -> Value {
    ...
    for value in values {
        if let Value::Number(x) = *value {
            ...
            return value_null!(
                "stddev: expected number, actual type is {} with value {}",
                value.type_of(),
                value
            );
        }
...
pub fn sublist2(list: &Value, position_value: &Value) -> Value {
    if let Value::List(items) = list {
        if let Value::Number(position_number) = position_value {
            if position_number.is_positive() {
                if let Ok(position) = <&FeelNumber as TryInto<usize>>::try_into(position_number) {
                    ...
                    if index < items.len() {
                        ...
                        value_null!(
                            "sublist: position is out of range, len = {}, position = {}",
                            items.len(),
                            position
                        )
                    }
                ...
                if let Ok(position) =
                    <FeelNumber as TryInto<usize>>::try_into(position_number.abs())
                {
                    let index = position;
                    if index <= items.len() {
            ...
            value_null!(
                "sublist: expected number, actual position value type is {}",
                position_value.type_of()
            )
        }
    } else {
        value_null!(
            "sublist: expected list, actual value type is {}",
            list.type_of()
        )
    }
}
        ...
        if let Value::Number(length_number) = length_value {
            if let Ok(length) = <&FeelNumber as TryInto<usize>>::try_into(length_number) {
                if let Value::Number(position_number) = position_value {
                    if position_number.is_positive() {
                        if let Ok(position) =
                            <&FeelNumber as TryInto<usize>>::try_into(position_number)
                        {
            ...
            value_null!(
                "sublist: expected number, actual length value type is {}",
                length_value.type_of()
            )
        }
    } else {
        value_null!(
...
pub fn substring(
    ...
    if let Value::String(input_string) = input_string_value {
        ...
        return value_null!("substring: invalid start position value: {}", start_position_number);
            ...
            return value_null!("substring: invalid length value: {}", length_number);
                    ...
                    match position.cmp(&0) {
                        Ordering::Greater => {
                            ...
                            if first < input_string_len && last <= input_string_len {
                                return Value::String(
                                    input_string.chars().skip(first).take(length).collect(),
                                );
                            } else {
                                value_null!(
                  ...
                  last + 1
                        ...
                        Ordering::Less => {
                            ...
                            if first >= 0
                                && (first as usize) < input_string_len
                                && (last as usize) <= input_string_len
                            {
                                return Value::String(
                                    input_string
                                        .chars()
                ...
                Value::Null(_) => match position.cmp(&0) {
                    Ordering::Greater => {
                        ...
                        if index < input_string_len {
                            ...
                            value_null!(
                                "substring: position is out of range, len = {}, position = {}",
                                input_string_len,
                    ...
                    Ordering::Less => {
                        ...
                        if index >= 0 {
                            return Value::String(
                                input_string.chars().skip(index as usize).collect(),
                            );
                        } else {
                            value_null!(
                                "substring: position is out of range, len = {}, position = {}",
                ...
                other => {
                    value_null!(
                        "sublist: expected number, actual length type is {}",
                        other.type_of()
                    )
                }
            }
            ...
            value_null!(
                "sublist: expected number, actual start position type is {}",
                start_position_value.type_of()
            )
        }
    } else {
        value_null!(
            "sublist: expected string, actual value type is {}",
            input_string_value.type_of()
        )
    }
}
...
pub fn substring_after(input_string_value: &Value, match_input_string: &Value) -> Value {
    if let Value::String(input_string) = input_string_value {
        if let Value::String(match_string) = match_input_string {
            ...
            value_null!(
                "substring after: expected string, actual match type is: {}",
                match_input_string.type_of()
            )
        }
    } else {
        value_null!(
            "substring after: expected string, actual input type is: {}",
            input_string_value.type_of()
        )
    }
}
...
pub fn substring_before(input_string_value: &Value, match_input_string: &Value) -> Value {
    if let Value::String(input_string) = input_string_value {
        if let Value::String(match_string) = match_input_string {
            ...
            value_null!(
                "substring before: expected string, actual match type is: {}",
                match_input_string.type_of()
            )
        }
    } else {
        value_null!(
            "substring before: expected string, actual input type is: {}",
            input_string_value.type_of()
        )
    }
}
    ...
    if let Value::Number(hour) = hour_value {
        if let Value::Number(minute) = minute_value {
            if let Value::Number(second) = second_value {
                ...
                value_null!(
                    "seconds must be a number, current type is {}",
                    second_value.type_of()
                )
            }
        } else {
            value_null!(
                "minutes must be a number, current type is {}",
                minute_value.type_of()
            )
        }
    } else {
        value_null!(
            "hour must be a number, current type is {}",
            hour_value.type_of()
        )
    }
}
            ...
            if let Value::Number(second) = second_value {
                if (0..24).contains(hour) {
                    if (0..60).contains(minute) {
                        if (0..60).contains(second) {
                            ...
                            let m = minute.try_into().unwrap();
                            let s = seconds.try_into().unwrap();
                            let n = (second.frac() * FeelNumber::billion())
                                .trunc()
                                .try_into()
                                .unwrap();
                            match offset_value {
                ...
                _ => value_null!("expected days and time duration or null, current offset type is {}", offset_value.type_of()),
                            ...
                            value_null!(
                                "core",
                                "time_4",
                                "second must be 0..59, current value is: {}",
                                second
                            )
                        }
                    } else {
                        value_null!(
                            "core",
                            "time_4",
                            "minute must be 0..59, current value is: {}",
                            minute
                        )
                    }
                } else {
                    value_null!(
                        "core",
                        "time_4",
                        "hour must be 0..23, current value is: {}",
                        hour
                    )
                }
            } else {
...
mod tests {
    ...
    fn test_gregorian_month_of_year() {
        assert_eq!(
            "null([month of year] no month)",
            gregorian_month(None).to_string()
        );
    }
}
