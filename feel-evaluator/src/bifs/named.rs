...
macro_rules! invalid_number_of_parameters {
    ($expected:expr, $actual:expr) => {{
        ...
        value_null!(
            "expected {} parameters, actual number of parameters is {}",
            $expected,
            $actual
        )
    }};
...
fn bif_after(parameters: &NamedParameters) -> Value {
    if let Some(((value1, _), (value2, _))) =
        get_param(parameters, &NAME_POINT_1).zip(get_param(parameters, &NAME_POINT_2))
    {
        core::after(value1, value2)
    } else if let Some(((value1, pos1), (value2, pos2))) =
        get_param(parameters, &NAME_POINT).zip(get_param(parameters, &NAME_RANGE))
    {
        if pos1 > pos2 {
    ...
    } else if let Some(((value1, _), (value2, _))) =
        get_param(parameters, &NAME_RANGE_1).zip(get_param(parameters, &NAME_RANGE_2))
    {
        core::after(value1, value2)
    } else {
...
fn bif_finished_by(parameters: &NamedParameters) -> Value {
        ...
        get_param(parameters, &NAME_RANGE_1).zip(get_param(parameters, &NAME_RANGE_2))
    {
        ...
        get_param(parameters, &NAME_RANGE).zip(get_param(parameters, &NAME_POINT))
    {
        core::finished_by(value1, value2)
    } else {
...
fn get_param<'a>(
    parameters: &'a NamedParameters,
    name: &'a Name,
) -> Option<(&'a Value, &'a usize)> {
