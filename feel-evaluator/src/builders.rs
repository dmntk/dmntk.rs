...
use dmntk_feel::{
    value_null, Evaluator, FeelNumber, FeelScope, FeelType, FunctionBody, Name, QualifiedName,
};
            ...
            Value::DaysAndTimeDuration(lh) => match rhv {
                ...
                Value::DateTime(rh) => {
                    if let Some(a) = rh + lh {
                        ...
                        value_null!(
                            "invalid result while adding date and time to days and time duration"
                        )
                    }
                }
...
fn build_at(_bx: &BuildContext, text: &str) -> Result<Evaluator> {
    if let Ok(date) = FeelDate::from_str(text) {
        ...
        Ok(Box::new(move |_: &FeelScope| {
            Value::DateTime(date_time.clone())
        }))
        ...
        Ok(Box::new(move |_: &FeelScope| {
            Value::YearsAndMonthsDuration(ym_duration.clone())
        }))
    } else if let Ok(dt_duration) = FeelDaysAndTimeDuration::try_from(text) {
        Ok(Box::new(move |_: &FeelScope| {
            Value::DaysAndTimeDuration(dt_duration.clone())
        }))
    } else {
        let invalid_at_literal = format!("invalid @ literal: {}", text);
        Ok(Box::new(move |_: &FeelScope| {
            value_null!(invalid_at_literal)
        }))
    }
}
...
fn build_between(
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match lhv {
            ...
            other => value_null!(
                "unexpected value type in 'between' operator: {}",
                other.type_of()
            ),
        }
    }))
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        for evaluator in &evaluators {
            match evaluator(scope) {
                ...
                other => {
                    return value_null!(
                        "expected context entry, actual value type is {}",
                        other.type_of()
                    )
                }
            }
...
fn build_context_entry(bx: &BuildContext, lhs: &AstNode, rhs: &AstNode) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match lhv {
            ...
            _ => value_null!(
                "expected context entry key, actual value type is {}",
                lhv.type_of()
            ),
        }
    }))
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        for evaluator in &evaluators {
            match evaluator(scope) {
                ...
                other => {
                    return value_null!(
                        "expected context type entry, actual value type is {}",
                        other.type_of()
                    )
                }
            }
...
fn build_context_type_entry_key(_bx: &BuildContext, lhs: &Name) -> Result<Evaluator> {
    let name = lhs.clone();
    Ok(Box::new(move |_: &FeelScope| {
        Value::ContextTypeEntryKey(name.clone())
    }))
}
...
fn build_feel_type(_bx: &BuildContext, lhs: &FeelType) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |_: &FeelScope| {
        Value::FeelType(feel_type.clone())
    }))
}
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match lhv {
            Value::List(values) => {
                ...
                for value in &values {
                    let (added_local_context, has_item_entry) =
                        if let Value::Context(local_context) = value {
                ...
                match rhv {
                    Value::Number(index) => {
                        if index.is_integer() {
                            ...
                            value_null!(
                                "index in filter must be an integer value, actual value is {}",
                                index
                            )
                        }
                    }
            ...
            match item {
                AstNode::IterationContextRange(variable_name, range_start_node, range_end_node) => {
                    ...
                    if let AstNode::Name(name) = variable_name.borrow() {
                        evaluators_range.push((
                            name.clone(),
                            evaluator_range_start,
                            evaluator_range_end,
                        ));
                    }
                }
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        if !evaluators_range.is_empty() {
            for (name, evaluator_range_start, evaluator_range_end) in &evaluators_range {
                expression_evaluator.add_range(
                    name.clone(),
                    evaluator_range_start(scope),
                    evaluator_range_end(scope),
                );
            }
        }
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        for evaluator in &evaluators {
            match evaluator(scope) {
                ...
                other => {
                    return value_null!(
                        "expected formal parameter, actual value type is: {}",
                        other.type_of()
                    )
                }
            }
...
fn build_internal_function_body(bx: &BuildContext, lhs: &AstNode) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |_: &FeelScope| {
        Value::FunctionBody(FunctionBody::LiteralExpression(lhe.clone()), false)
    }))
}
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match mapping_value {
            Value::Context(mapping_information) => {
                if let Some(Value::Context(java_mapping)) =
                    mapping_information.get_entry(&"java".into())
                {
                    return if let Some(Value::String(class_name)) =
                        java_mapping.get_entry(&"class".into())
                    {
                        if let Some(Value::String(method_signature)) =
                            java_mapping.get_entry(&"method signature".into())
                        {
                            let java_class_name = class_name.to_owned();
                            let java_method_signature = method_signature.to_owned();
                            let java_evaluator = Box::new(move |_: &FeelScope| {
                                Value::ExternalJavaFunction(
                                    java_class_name.clone(),
                                    java_method_signature.clone(),
                                )
                            }) as Evaluator;
                        ...
                        value_null!(
                            "invalid Java function mapping, no class name entry in context {}",
                            java_mapping
                        )
                    };
                }
                if let Some(Value::Context(pmml_mapping)) =
                    mapping_information.get_entry(&"pmml".into())
                {
                    return if let Some(Value::String(document)) =
                        pmml_mapping.get_entry(&"document".into())
                    {
                        if let Some(Value::String(model_name)) =
                            pmml_mapping.get_entry(&"model".into())
                        {
                            let pmml_document = document.to_owned();
                            let pmml_model_name = model_name.to_owned();
                            let pmml_evaluator = Box::new(move |_: &FeelScope| {
                                Value::ExternalPmmlFunction(
                                    pmml_document.clone(),
                                    pmml_model_name.clone(),
                                )
                            }) as Evaluator;
                            ...
                            value_null!(
                                "invalid PMML function mapping, no model name entry in context {}",
                                pmml_mapping
                            )
                        }
                    } else {
                        value_null!(
                            "invalid PMML function mapping, no document entry in context {}",
                            pmml_mapping
                        )
                    };
                }
            ...
            other => value_null!(
                "invalid external function mapping, expected context, actual value is {}",
                other
            ),
        }
    }))
...
fn build_function_definition(bx: &BuildContext, lhs: &AstNode, rhs: &AstNode) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match lhv {
            ...
            other => value_null!(
                "expected formal parameters, actual value type is {}",
                other.type_of()
            ),
        }
    }))
...
fn build_function_invocation(bx: &BuildContext, lhs: &AstNode, rhs: &AstNode) -> Result<Evaluator> {
    match rhs {
        AstNode::PositionalParameters(parameters) => {
            build_function_invocation_with_positional_parameters(bx, lhs, parameters)
        }
        node @ AstNode::NamedParameters(_) => {
            build_function_invocation_with_named_parameters(bx, lhs, node)
        }
...
///
fn build_function_invocation_with_positional_parameters(
    bx: &BuildContext,
    lhs: &AstNode,
    rhs: &[AstNode],
) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        let args = argument_evaluators
            .iter()
            .map(|evaluator| evaluator(scope))
            .collect::<Vec<Value>>();
        match function {
            ...
            Value::FunctionDefinition(params, body, external, _, closure_ctx, result_type) => {
                if external {
                    eval_external_function_with_positional_parameters(
                        scope,
                        &args,
                        &params,
                        &body,
                    ...
                    eval_function_with_positional_parameters(
                        scope,
                        &args,
                        &params,
                        &body,
            ...
            _ => value_null!(
                "expected built-in function name or function definition, actual is {}",
                function
            ),
        }
    }))
...
///
fn build_function_invocation_with_named_parameters(
    bx: &BuildContext,
    lhs: &AstNode,
    rhs: &AstNode,
) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match function {
            ...
            Value::FunctionDefinition(params, body, external, _, closure_ctx, result_type) => {
                if external {
                    eval_external_function_with_named_parameters(
                        scope,
                        &args,
                        &params,
                        &body,
                    ...
                    eval_function_with_named_parameters(
                        scope,
                        &args,
                        &params,
                        &body,
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match rhv {
            ...
            Value::Range(l, l_closed, r, r_closed) => {
                eval_in_range(&lhv, &l, l_closed, &r, r_closed)
            }
            Value::List(r_inner) => {
            ...
            Value::UnaryGreaterOrEqual(inner) => {
                eval_in_unary_greater_or_equal(&lhv, inner.borrow())
            }
            Value::Irrelevant => VALUE_TRUE,
            _ => value_null!(
                "unexpected argument type in 'in' operator: {}",
                rhv.type_of()
            ),
        }
    }))
...
fn build_mul(bx: &BuildContext, lhs: &AstNode, rhs: &AstNode) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match lhv {
                ...
                match rhv {
                    ...
                    Value::DaysAndTimeDuration(ref rh) => {
                        ...
                        if let Ok(v) = FeelNumber::try_into(val) {
                            ...
                            value_null!(
                                "multiplication result is out of range of days and time duration"
                            )
                        }
                    }
            ...
            Value::YearsAndMonthsDuration(ref lh) => {
                match rhv {
                    Value::Number(rh) => {
            ...
            value @ Value::Null(_) => value,
            other => value_null!(
                "unexpected value type in multiplication: {}",
                other.type_of()
            ),
        }
    }))
...
fn build_neg(bx: &BuildContext, lhs: &AstNode) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match lhv {
            ...
            other => value_null!(
                "unexpected type in arithmetic negation: {}",
                other.type_of()
            ),
        }
    }))
...
fn build_numeric(_bx: &BuildContext, lhs: &str, rhs: &str) -> Result<Evaluator> {
    ...
    if let Ok(num) = text.parse::<FeelNumber>() {
        ...
        Ok(Box::new(move |_: &FeelScope| {
            value_null!("failed to convert text '{}' into number", text)
        }))
    }
}
...
fn build_qualified_name(bx: &BuildContext, lhs: &[AstNode]) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        scope
            .search(&names)
            .unwrap_or_else(|| value_null!("no value for qualified name"))
    }))
}
...
fn build_qualified_name_segment(_bx: &BuildContext, name: &Name) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |_: &FeelScope| {
        Value::QualifiedNameSegment(name.to_owned())
    }))
}
    ...
    match node {
        AstNode::Path(lhs, rhs) => {
            if let AstNode::Name(name) = lhs.borrow() {
                ...
                Err(err_unexpected_ast_node(&format!(
                    "expected Name, found {lhs:?}"
                )))
            }
        }
        AstNode::Name(name) => Ok(name.clone().into()),
        _ => Err(err_unexpected_ast_node(&format!(
            "expected Path or Name, found: {node:?}"
        ))),
    }
}
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        match lhv {
            Value::Context(context) => {
                ...
                value_null!(
                    "build_path: no entry {} in context: {}",
                    qualified_name,
                    context
                )
            }
...
fn build_sub(bx: &BuildContext, lhs: &AstNode, rhs: &AstNode) -> Result<Evaluator> {
    ...
    Ok(Box::new(move |scope: &FeelScope| {
        ...
        value_null!(
            "[subtraction] incompatible types: {} - {}",
            lhe(scope) as Value,
            rhe(scope) as Value
        )
    }))
...
pub fn eval_ternary_equality(lhs: &Value, rhs: &Value) -> Option<bool> {
    match lhs {
        ...
        ls @ Value::Null(_) => match rhs {
            rs @ Value::Null(_) => {
                if (ls.is_invalid_coercion() && !rs.is_invalid_coercion())
                    || (!ls.is_invalid_coercion() && rs.is_invalid_coercion())
                {
                    None
                } else {
                    Some(true)
    ...
    if let Value::NamedParameters(argument_map) = args {
        ...
        for (parameter_name, parameter_type) in params {
            if let Some((argument, _)) = argument_map.get(parameter_name) {
                ...
                return value_null!(
                    "parameter with name {} not found in arguments",
                    parameter_name
                );
            }
        }
...
fn eval_function_definition(
    ...
    if let Value::FunctionDefinition(
        fd_params,
        fd_body,
        fd_external,
        fd_closure,
...
fn eval_external_function_with_positional_parameters(
    scope: &FeelScope,
    args: &[Value],
    params: &[(Name, FeelType)],
    body: &FunctionBody,
...
fn eval_external_function_with_named_parameters(
    scope: &FeelScope,
    args: &Value,
    params: &[(Name, FeelType)],
    body: &FunctionBody,
...
fn eval_external_function_definition(
    scope: &FeelScope,
    arguments: &[Value],
    body: &FunctionBody,
    result_type: FeelType,
    ...
    let result = match &body.evaluate(scope) {
        Value::ExternalJavaFunction(class_name, method_signature) => {
            evaluate_external_java_function(class_name, method_signature, arguments)
        }
        Value::ExternalPmmlFunction(document, model_name) => {
            evaluate_external_pmml_function(document, model_name, arguments)
        }
...
mod tests {
    ...
    fn test_unimplemented_external_function_kind() {
        ...
        assert_eq!(
            "null(expected JAVA or PMML mapping, actual value is false)",
            result.to_string()
        )
    }
}
