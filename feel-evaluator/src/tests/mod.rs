...
use dmntk_feel_temporal::{
    Day, FeelDate, FeelDateTime, FeelDaysAndTimeDuration, FeelTime, FeelYearsAndMonthsDuration,
    Month, Year,
};
...
pub fn te_date_time_local(
    ...
    textual_expression(
        ...
        scope,
        s,
        Value::DateTime(FeelDateTime::local(year, month, day, hour, min, sec, nano)),
    );
}

/// Utility function that tests evaluation of UTC date and time value.
pub fn te_date_time_utc(
    trace: bool,
    scope: &FeelScope,
    s: &str,
    date: (Year, Month, Day),
    ...
    textual_expression(
        ...
        scope,
        s,
        Value::DateTime(FeelDateTime::utc(year, month, day, hour, min, sec, nano)),
    );
}

/// Utility function that tests evaluation of date and time value with explicit offset.
pub fn te_date_time_offset(
    trace: bool,
    scope: &FeelScope,
    s: &str,
    date: (Year, Month, Day),
    ...
    textual_expression(
        ...
        scope,
        s,
        Value::DateTime(FeelDateTime::offset(date, time, offset)),
    );
}
...
pub fn te_number_x(trace: bool, scope: &FeelScope, s: &str, num: &str) {
    textual_expression(
        ...
        scope,
        s,
        Value::Number(num.parse::<FeelNumber>().unwrap()),
    );
}
...
pub fn te_years_and_months_duration(
    trace: bool,
    scope: &FeelScope,
    s: &str,
    years: i64,
...
pub fn te_years_and_months_duration_x(trace: bool, scope: &FeelScope, s: &str, expected: &str) {
    textual_expression(
        ...
        scope,
        s,
        Value::YearsAndMonthsDuration(expected.try_into().unwrap()),
    );
}

/// Utility function that tests evaluation of days and time duration.
pub fn te_days_and_time_duration(
    trace: bool,
    scope: &FeelScope,
    s: &str,
    neg: bool,
...
pub fn te_days_and_time_duration_x(trace: bool, scope: &FeelScope, s: &str, expected: &str) {
    textual_expression(
        ...
        scope,
        s,
        Value::DaysAndTimeDuration(expected.try_into().unwrap()),
    );
}
...
fn textual_expression(trace: bool, scope: &FeelScope, text: &str, expected: Value) {
    match dmntk_feel_parser::parse_textual_expression(scope, text, trace) {
        Ok(node) => match build_evaluator(&BuildContext::default(), &node) {
            ...
            Err(reason) => {
                panic!(
                    "building evaluator for textual expression failed with reason: {}",
                    reason
                );
            }
        },
...
pub fn valid_unary_tests(trace: bool, scope: &FeelScope, text: &str) {
    match dmntk_feel_parser::parse_unary_tests(scope, text, trace) {
        Ok(node) => match build_evaluator(&BuildContext::default(), &node) {
            ...
            Err(reason) => {
                panic!(
                    "building evaluator for unary tests failed with reason: {}",
                    reason
                );
            }
        },
...
pub fn satisfies(
    ...
    let input_expression_node =
        dmntk_feel_parser::parse_textual_expression(scope, input_expression, trace).unwrap();
    let input_entry_node = dmntk_feel_parser::parse_unary_tests(scope, input_entry, trace).unwrap();
    let node = if !input_values.is_empty() {
        let input_values_node =
            dmntk_feel_parser::parse_unary_tests(scope, input_values, trace).unwrap();
        let left = AstNode::In(
            Box::new(input_expression_node.clone()),
            Box::new(input_values_node),
        );
