...
fn _0001() {
    let node = AstNode::FormalParameter(
        Box::new(AstNode::ParameterName("n".into())),
...
fn _0002() {
    let node = AstNode::FormalParameter(
        Box::new(AstNode::Name("n".into())),
        Box::new(AstNode::FeelType(FeelType::Boolean)),
    );
    let result = crate::evaluate(&scope!(), &node);
    assert_eq!(
        "null(expected name of the formal parameter)",
...
#[test]
fn _0003() {
    let parameter_x = AstNode::FormalParameter(
        Box::new(AstNode::ParameterName("x".into())),
        Box::new(AstNode::FeelType(FeelType::Number)),
    );
    let parameter_y = AstNode::FormalParameter(
        Box::new(AstNode::ParameterName("y".into())),
        Box::new(AstNode::FeelType(FeelType::Number)),
    );
