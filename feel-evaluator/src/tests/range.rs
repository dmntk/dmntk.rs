...
fn _0024() {
    te_null(
        false,
        &scope!(),
        r#" @"2023-02-06" in ["2023-02-01"..@"2023-02-28"]"#,
        "eval_in_range",
    );
}
...
fn _0028() {
    te_null(
        false,
        &scope!(),
        r#" @"10:11:12" in ["00:00:00"..@"23:59:59"]"#,
        "eval_in_range",
    );
}
...
fn _0035() {
    let node = AstNode::Range(
        Box::new(AstNode::IntervalStart(
            Box::new(AstNode::Numeric("1".into(), "0".into())),
            false,
        )),
        Box::new(AstNode::IntervalEnd(
            Box::new(AstNode::Numeric("5".into(), "0".into())),
            false,
        )),
    );
...
fn _0036() {
    ...
    let result = crate::evaluate(&scope!(), &node);
    assert_eq!(
        r#"null(expected interval start)"#,
