...
impl Definitions {
    ...
    pub fn performance_indicators(&self) -> Vec<&PerformanceIndicator> {
        self.business_context_elements
            ...
            .filter_map(|item| match item {
                BusinessContextElementInstance::PerformanceIndicator(performance_indicator) => {
    ...
    pub fn organisation_units(&self) -> Vec<&OrganizationUnit> {
        self.business_context_elements
            .iter()
            .filter_map(|item| match item {
                BusinessContextElementInstance::OrganizationUnit(organisation_unit) => {
                    Some(organisation_unit)
                }
                _ => None,
            })
            .collect()
