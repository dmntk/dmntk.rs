    ...
    match roxmltree::Document::parse(xml) {
        Ok(document) => {
            ...
            if definitions_node.tag_name().name() != NODE_DEFINITIONS {
                return Err(err_xml_unexpected_node(
                    NODE_DEFINITIONS,
                    definitions_node.tag_name().name(),
                ));
            }
...
impl ModelParser {
    ...
    fn parse_input_data(&self, node: &Node) -> Result<Vec<DrgElement>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_INPUT_DATA)
        {
    ...
    fn parse_decisions(&mut self, node: &Node) -> Result<Vec<DrgElement>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_DECISION)
        {
            let decision = Decision {
                ...
                information_requirements: self
                    .parse_information_requirements(child_node, NODE_INFORMATION_REQUIREMENT)?,
                knowledge_requirements: self
                    .parse_knowledge_requirements(child_node, NODE_KNOWLEDGE_REQUIREMENT)?,
                authority_requirements: self
                    .parse_authority_requirements(child_node, NODE_AUTHORITY_REQUIREMENT)?,
            };
    ...
    fn parse_bkm(&mut self, node: &Node) -> Result<Vec<DrgElement>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_BUSINESS_KNOWLEDGE_MODEL)
        {
            let business_knowledge_model = BusinessKnowledgeModel {
                ...
                extension_attributes: self.parse_extension_attributes(child_node),
                variable: self.parse_information_item_child(child_node, NODE_VARIABLE)?,
                encapsulated_logic: self
                    .parse_function_definition_child(child_node, NODE_ENCAPSULATED_LOGIC)?,
                knowledge_requirements: self
                    .parse_knowledge_requirements(child_node, NODE_KNOWLEDGE_REQUIREMENT)?,
                authority_requirements: self
                    .parse_authority_requirements(child_node, NODE_AUTHORITY_REQUIREMENT)?,
    ...
    fn parse_decision_services(&self, node: &Node) -> Result<Vec<DrgElement>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_DECISION_SERVICE)
        {
            let decision_service = DecisionService {
                ...
                extension_attributes: self.parse_extension_attributes(child_node),
                variable: self.parse_information_item_child(child_node, NODE_VARIABLE)?,
                output_decisions: self
                    .required_hrefs_in_child_nodes(child_node, NODE_OUTPUT_DECISION)?,
                encapsulated_decisions: self
                    .required_hrefs_in_child_nodes(child_node, NODE_ENCAPSULATED_DECISION)?,
                input_decisions: self
                    .required_hrefs_in_child_nodes(child_node, NODE_INPUT_DECISION)?,
    ...
    fn parse_knowledge_sources(&mut self, node: &Node) -> Result<Vec<DrgElement>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_KNOWLEDGE_SOURCE)
        {
    ...
    ///
    fn parse_function_definition_child(
        &self,
        node: &Node,
        child_name: &str,
    ...
    fn parse_optional_function_definition(
        ...
        if let Some(child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_FUNCTION_DEFINITION)
        {
            Ok(Some(self.parse_function_definition(&child_node)?))
        } else {
            Ok(None)
    ...
    fn parse_function_definition(&self, node: &Node) -> Result<FunctionDefinition> {
        Ok(FunctionDefinition {
            ...
            formal_parameters: self
                .parse_information_items_from_child(node, NODE_FORMAL_PARAMETER)?,
    ...
    fn parse_business_context_elements(
        &self,
        node: &Node,
    ) -> Result<Vec<BusinessContextElementInstance>> {
        let mut business_context_elements = vec![];
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_PERFORMANCE_INDICATOR)
        {
            let performance_indicator = PerformanceIndicator {
                ...
                impacting_decisions: optional_children_required_href(
                    child_node,
                    NODE_IMPACTING_DECISION,
                )?,
            };
            business_context_elements.push(BusinessContextElementInstance::PerformanceIndicator(
                performance_indicator,
            ));
        }
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_ORGANISATION_UNIT)
        {
            ...
            business_context_elements.push(BusinessContextElementInstance::OrganizationUnit(
                organisation_unit,
            ));
        }
    ...
    fn parse_imports(&self, node: &Node) -> Result<Vec<Import>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_IMPORT)
        {
            let import = Import {
                id: optional_id(child_node),
                description: optional_child_optional_content(child_node, NODE_DESCRIPTION),
    ...
    fn parse_information_item_child(
        &self,
        node: &Node,
        child_name: &str,
    ) -> Result<InformationItem> {
        if let Some(child_node) = node.children().find(|n| n.tag_name().name() == child_name) {
            self.parse_information_item(&child_node)
        } else {
            Err(err_xml_expected_mandatory_child_node(
                &node_name_pos(node),
                child_name,
            ))
        }
    ...
    fn parse_optional_information_item_child(
        &self,
        node: &Node,
        child_name: &str,
    ) -> Result<Option<InformationItem>> {
        if let Some(child_node) = node.children().find(|n| n.tag_name().name() == child_name) {
    ...
    fn parse_information_items_from_child(
        &self,
        node: &Node,
        child_name: &str,
    ) -> Result<Vec<InformationItem>> {
    ...
    fn parse_information_item(&self, node: &Node) -> Result<InformationItem> {
        Ok(InformationItem {
            ...
            feel_name: required_feel_name(node)?,
            type_ref: optional_attribute(node, ATTR_TYPE_REF)
                .unwrap_or(FEEL_TYPE_NAME_ANY.to_string()),
            feel_type: None,
        })
    }

    /// Parses information requirements.
    fn parse_information_requirements(
        &mut self,
        node: &Node,
        child_name: &str,
    ) -> Result<Vec<InformationRequirement>> {
    ...
    fn parse_knowledge_requirements(
        &mut self,
        node: &Node,
        child_name: &str,
    ) -> Result<Vec<KnowledgeRequirement>> {
    ...
    fn parse_required_child_expression_instance(&self, node: &Node) -> Result<ExpressionInstance> {
        self.parse_optional_child_expression_instance(node)?
            .ok_or_else(err_required_expression_instance_is_missing)
    }

    fn parse_optional_child_expression_instance(
        &self,
        node: &Node,
    ) -> Result<Option<ExpressionInstance>> {
        ...
        if let Some(decision_table) = self.parse_optional_decision_table(node)? {
            return Ok(Some(ExpressionInstance::DecisionTable(Box::new(
                decision_table,
            ))));
        }
        if let Some(function_definition) = self.parse_optional_function_definition(node)? {
            return Ok(Some(ExpressionInstance::FunctionDefinition(Box::new(
                function_definition,
            ))));
        }
        ...
        if let Some(literal_expression) = self.parse_optional_literal_expression(node) {
            return Ok(Some(ExpressionInstance::LiteralExpression(Box::new(
                literal_expression,
            ))));
        }
    ...
    fn parse_required_expression_instance(&self, node: &Node) -> Result<ExpressionInstance> {
        match node.tag_name().name() {
            ...
            NODE_FUNCTION_DEFINITION => {
                ...
                Ok(ExpressionInstance::FunctionDefinition(Box::new(
                    function_definition,
                )))
            }
            NODE_INVOCATION => {
            ...
            NODE_LITERAL_EXPRESSION => {
                ...
                Ok(ExpressionInstance::LiteralExpression(Box::new(
                    literal_expression,
                )))
            }
            NODE_RELATION => {
    ...
    fn parse_optional_decision_table(&self, node: &Node) -> Result<Option<DecisionTable>> {
        if let Some(ref child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_DECISION_TABLE)
        {
    ...
    fn parse_decision_table_inputs(&self, node: &Node) -> Result<Vec<InputClause>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_INPUT)
        {
    ...
    fn parse_decision_table_input(&self, node: &Node) -> Result<InputClause> {
        let input_expression =
            if let Ok(ref child_node) = required_child(node, NODE_INPUT_EXPRESSION) {
    ...
    fn parse_decision_table_outputs(&self, node: &Node) -> Result<Vec<OutputClause>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_OUTPUT)
        {
    ...
    fn parse_decision_table_output(&self, node: &Node) -> Result<OutputClause> {
        ...
        let default_output_entry =
            if let Some(ref child_node) = optional_child(node, NODE_DEFAULT_OUTPUT_ENTRY) {
                optional_child_required_content(child_node, NODE_TEXT)?
            } else {
                None
            };
    ...
    fn parse_decision_table_input_entries(&self, node: &Node) -> Result<Vec<InputEntry>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_INPUT_ENTRY)
        {
    ...
    fn parse_decision_table_output_entries(&self, node: &Node) -> Result<Vec<OutputEntry>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_OUTPUT_ENTRY)
        {
    ...
    fn parse_optional_context(&self, node: &Node) -> Result<Option<Context>> {
        if let Some(ref child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_CONTEXT)
        {
    ...
    fn parse_context_entries(&self, node: &Node) -> Result<Vec<ContextEntry>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_CONTEXT_ENTRY)
        {
    ...
    fn parse_optional_invocation(&self, node: &Node) -> Result<Option<Invocation>> {
        if let Some(ref child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_INVOCATION)
        {
    ...
    fn parse_bindings(&self, node: &Node) -> Result<Vec<Binding>> {
        ...
        for ref child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_BINDING)
        {
    ...
    fn parse_optional_literal_expression(&self, node: &Node) -> Option<LiteralExpression> {
        if let Some(ref child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_LITERAL_EXPRESSION)
        {
    ...
    fn parse_optional_relation(&self, node: &Node) -> Result<Option<Relation>> {
        if let Some(ref child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_RELATION)
        {
    ...
    fn parse_relation(&self, node: &Node) -> Result<Relation> {
        let mut columns = vec![];
        for ref column_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_COLUMN)
        {
            ...
            for ref expression_instance_node in row_node.children() {
                if expression_instance_node.tag_name().name() == NODE_LITERAL_EXPRESSION {
                    let literal_expression =
                        self.parse_literal_expression(expression_instance_node);
                    elements.push(ExpressionInstance::LiteralExpression(Box::new(
                        literal_expression,
                    )));
                }
            }
    ...
    fn parse_preferred_orientation_attribute(
        &self,
        node: &Node,
    ) -> Result<DecisionTableOrientation> {
    ...
    fn parse_styles(&self, node: &Node) -> Result<Vec<DmnStyle>> {
        ...
        for child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_DMNDI_STYLE)
        {
    ...
    fn parse_style(&self, node: &Node) -> Result<DmnStyle> {
        Ok(DmnStyle {
            ...
            label_horizontal_alignment: self
                .parse_alignment_kind(node, NODE_DMNDI_LABEL_HORIZONTAL_ALIGNMENT),
            label_vertical_alignment: self
                .parse_alignment_kind(node, NODE_DMNDI_LABEL_VERTICAL_ALIGNMENT),
        })
    }
    ...
    fn parse_diagrams(&self, node: &Node) -> Result<Vec<DmnDiagram>> {
        ...
        for child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_DMNDI_DMN_DIAGRAM)
        {
    ...
    fn parse_dimension(&self, size_node: &Node) -> Result<Option<DcDimension>> {
        if let Some(node) = size_node
            .children()
            .find(|n| n.tag_name().name() == NODE_DMNDI_SIZE)
        {
            Ok(Some(DcDimension {
    ...
    fn parse_diagram_elements(&self, size_node: &Node) -> Result<Vec<DmnDiagramElement>> {
        ...
        for child_node in size_node
            .children()
            .filter(|n| n.tag_name().name() == NODE_DMNDI_DMN_SHAPE)
        {
            diagram_element.push(self.parse_shape(&child_node)?);
        }
        for child_node in size_node
            .children()
            .filter(|n| n.tag_name().name() == NODE_DMNDI_DMN_EDGE)
        {
    ...
    fn parse_bounds(&self, node: &Node) -> Result<DcBounds> {
        match self.parse_optional_bounds(node) {
            Ok(Some(n)) => Ok(n),
            _ => Err(err_required_child_node_is_missing(
                node.tag_name().name(),
                NODE_DMNDI_BOUNDS,
            )),
        }
    ...
    fn parse_optional_bounds(&self, node: &Node) -> Result<Option<DcBounds>> {
        if let Some(child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_DMNDI_BOUNDS)
        {
            Ok(Some(DcBounds {
    ...
    fn parse_divider_line(&self, node: &Node) -> Result<Option<DmnDecisionServiceDividerLine>> {
        if let Some(child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_DMNDI_DECISION_SERVICE_DIVIDER_LINE)
        {
    ...
    fn parse_way_points(&self, node: &Node) -> Result<Vec<DcPoint>> {
        ...
        for child_node in node
            .children()
            .filter(|n| n.tag_name().name() == NODE_DMNDI_WAYPOINT)
        {
    ...
    fn parse_label(&self, node: &Node) -> Result<Option<DmnLabel>> {
        if let Some(child_node) = node
            .children()
            .find(|n| n.tag_name().name() == NODE_DMNDI_LABEL)
        {
            Ok(Some(DmnLabel {
...
fn optional_id(node: &Node) -> DmnId {
    optional_attribute(node, ATTR_ID)
        .map(DmnId::Provided)
        .unwrap_or(DmnId::Generated(gen_id()))
}
...
pub fn optional_child_required_href(node: &Node, child_name: &str) -> Result<Option<HRef>> {
    if let Some(child_node) = optional_child(node, child_name) {
        Ok(Some(HRef::try_from(
            required_attribute(&child_node, ATTR_HREF)?.as_str(),
        )?))
    } else {
        Ok(None)
    }
