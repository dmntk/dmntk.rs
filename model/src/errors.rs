...
pub fn err_invalid_function_kind(s: &str) -> DmntkError {
    ModelParserError(format!(
        "'{s}' is not a valid function kind, accepted values are: 'FEEL', 'Java', 'PMML'"
    ))
    .into()
}
...
pub fn err_invalid_aggregation(s: &str) -> DmntkError {
    ModelParserError(format!(
        "'{s}' is not a valid aggregation, allowed values are: 'COUNT', 'SUM', 'MIN', 'MAX'"
    ))
    .into()
}
...
pub fn err_invalid_color_value(s: &str) -> DmntkError {
    ModelParserError(format!(
        "conversion to valid color value failed with reason: {s}"
    ))
    .into()
}
...
pub fn err_invalid_double_value(reason: &str) -> DmntkError {
    ModelParserError(format!(
        "conversion to valid double value failed with reason: {reason}"
    ))
    .into()
}
...
pub fn err_required_child_node_is_missing(s1: &str, s2: &str) -> DmntkError {
    ModelParserError(format!(
        "required child node '{s2}' in parent node '{s1}' is missing"
    ))
    .into()
}
...
pub fn err_required_input_expression_is_missing() -> DmntkError {
    ModelParserError(
        "required input expression in decision table's input clause is missing".to_string(),
    )
    .into()
}
...
pub fn err_number_of_elements_in_row_differs_from_number_of_columns() -> DmntkError {
    ModelParserError(
        "number of elements in a row differs from the number of columns defined in a relation"
            .to_string(),
    )
    .into()
}
...
pub fn err_xml_expected_mandatory_attribute(s1: &str, s2: &str) -> DmntkError {
    ModelParserError(format!(
        "expected value for mandatory attribute '{s2}' in node {s1}"
    ))
    .into()
}
...
pub fn err_xml_expected_mandatory_child_node(s1: &str, s2: &str) -> DmntkError {
    ModelParserError(format!(
        "expected mandatory child node '{s2}' in parent node {s1}"
    ))
    .into()
}
