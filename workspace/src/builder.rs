...
impl WorkspaceBuilder {
    ...
    pub fn load_decision_models(&mut self, dir: &Path) {
        ...
        for (workspace_name, loaded_definitions) in &self.workspace_definitions {
            match ModelEvaluator::new(loaded_definitions) {
                Ok(evaluator) => {
                    self.evaluators
                        .insert(workspace_name.to_string(), evaluator);
                }
                Err(reason) => {
        ...
        for (workspace_name, evaluator) in &self.evaluators {
            for (namespace, invocable_name) in evaluator.invocables().list() {
                ...
                self.invocables.insert(
                    invocable_path.clone(),
                    (workspace_name.clone(), namespace, invocable_name),
                );
            }
        }
        // display summary
    ...
    fn display_summary(&self) {
        ...
        let deployed_invocables_count = self
            .evaluators
            .values()
            .map(|evaluator| evaluator.invocables().len())
            .sum();
        println!(
            "{1}Deployed {2} {3}.{0}",
            self.colors.reset(),
