...
fn write_model(w: &mut dyn Write, definitions: &Definitions, colors: &Colors, indent: usize) {
    ...
    let node_model = AsciiNode::node_builder(
        AsciiLine::builder()
            .with_color(definitions.name(), colors.name())
            .build(),
    )
    ...
    .opt_child(build_labeled_uri(
        LABEL_EXPRESSION_LANGUAGE,
    ...
    .opt_child(build_opt_labeled_text(
        "exporter",
    ...
    .opt_child(build_extension_elements(
        definitions.extension_elements(),
        colors,
    ))
    .opt_child(build_extension_attributes(
        definitions.extension_attributes(),
        colors,
    ))
    .build();
...
fn write_decisions(w: &mut dyn Write, definitions: &Definitions, colors: &Colors, indent: usize) {
    ...
    for (i, decision) in decisions.iter().enumerate() {
        let node_decision = builder_name(decision.name(), colors)
            ...
            .opt_child(build_opt_labeled_multiline_text(
                LABEL_QUESTION,
                decision.question(),
                colors.default(),
            ))
            .opt_child(build_opt_labeled_multiline_text(
                LABEL_ALLOWED_ANSWERS,
                decision.allowed_answers(),
                colors.default(),
            ))
            .opt_child(build_description(decision.description(), colors))
            .child(build_variable(decision.variable(), colors))
            .opt_child(build_authority_requirements(
                decision.authority_requirements(),
                colors,
            ))
            .opt_child(build_extension_elements(
                decision.extension_elements(),
                colors,
            ))
            .opt_child(build_extension_attributes(
                decision.extension_attributes(),
                colors,
            ))
            .build();
...
fn write_business_knowledge_models(
    w: &mut dyn Write,
...
fn write_decision_services(
    ...
    for (i, decision_service) in decision_services.iter().enumerate() {
        let node_decision = builder_name(decision_service.name(), colors)
            ...
            .opt_child(build_extension_elements(
                decision_service.extension_elements(),
                colors,
            ))
            .opt_child(build_extension_attributes(
                decision_service.extension_attributes(),
                colors,
            ))
            .build();
        write_empty_line(w, i);
...
fn write_knowledge_sources(
    ...
    for (i, knowledge_source) in knowledge_sources.iter().enumerate() {
        let node_decision = builder_name(knowledge_source.name(), colors)
            ...
            .opt_child(build_extension_elements(
                knowledge_source.extension_elements(),
                colors,
            ))
            .opt_child(build_extension_attributes(
                knowledge_source.extension_attributes(),
                colors,
            ))
            .opt_child(build_authority_requirements(
                knowledge_source.authority_requirements(),
                colors,
            ))
            .build();
        write_empty_line(w, i);
...
fn write_performance_indicators(
    w: &mut dyn Write,
    definitions: &Definitions,
    colors: &Colors,
    indent: usize,
    ...
    for (i, performance_indicator) in performance_indicators.iter().enumerate() {
        ...
        let mut impacting_decisions_builder = AsciiNode::node_builder(
            AsciiLine::builder()
                .text(LABEL_IMPACTING_DECISIONS)
                .colon()
                .build(),
        );
        ...
        let node_performance_indicator = builder_name(performance_indicator.name(), colors)
            ...
            .opt_child(build_extension_elements(
                performance_indicator.extension_elements(),
                colors,
            ))
            .opt_child(build_extension_attributes(
                performance_indicator.extension_attributes(),
                colors,
            ))
            .build();
        write_empty_line(w, i);
...
fn write_organisation_units(
    ...
    for (i, organisation_units) in definitions.organisation_units().iter().enumerate() {
        ...
        let mut decisions_made_builder = AsciiNode::node_builder(
            AsciiLine::builder()
                .text(LABEL_DECISIONS_MADE)
                .colon()
                .build(),
        );
        ...
        let node_organisation_unit = builder_name(organisation_units.name(), colors)
            ...
            .opt_child(build_extension_elements(
                organisation_units.extension_elements(),
                colors,
            ))
            .opt_child(build_extension_attributes(
                organisation_units.extension_attributes(),
                colors,
            ))
            .build();
        write_empty_line(w, i);
...
fn build_href(href: &HRef, colors: &Colors) -> AsciiNode {
    ...
    AsciiNode::leaf_builder()
        .line(
            AsciiLine::builder()
                .with_color("#", colors.href())
                .with_color(text, colors.href())
                .build(),
        )
        .build()
}
...
fn build_extension_elements(
    _extension_elements: &[ExtensionElement],
    _colors: &Colors,
) -> Option<AsciiNode> {
    None
}

fn build_extension_attributes(
    _extension_attributes: &[ExtensionAttribute],
    _colors: &Colors,
) -> Option<AsciiNode> {
    None
}

fn build_authority_requirements(
    authority_requirements: &Vec<AuthorityRequirement>,
    colors: &Colors,
) -> Option<AsciiNode> {
    if !authority_requirements.is_empty() {
        let mut authority_requirements_builder =
            AsciiNode::node_builder(AsciiLine::builder().text("authority requirements:").build());
        for authority_requirement in authority_requirements {
            let a =
                AsciiNode::node_builder(AsciiLine::builder().text("authority requirement").build())
                    .opt_child(build_id(authority_requirement.opt_id(), colors))
                    .opt_child(build_label(authority_requirement.label(), colors))
                    .opt_child(build_description(
                        authority_requirement.description(),
                        colors,
                    ))
                    .opt_child(build_extension_elements(
                        authority_requirement.extension_elements(),
                        colors,
                    ))
                    .opt_child(build_extension_attributes(
                        authority_requirement.extension_attributes(),
                        colors,
                    ))
                    .build();
...
fn build_opt_labeled_multiline_text(
    label: &str,
    opt_text: &Option<String>,
    color: &str,
) -> Option<AsciiNode> {
    opt_text.as_ref().map(|text| {
        ...
        for line in text
            ...
            .map(|s| s[min_spaces..].trim_end())
        {
            leaf_builder.add_line(
                AsciiLine::builder()
                    .indent()
