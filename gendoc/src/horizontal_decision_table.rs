...
pub fn create_horizontal_decision_table_elements(decision_table: &DecisionTable) -> HtmlElement {
    ...
    html_first_vertical_double_line
        .set_style(&prepare_style_first_vertical_double_line(decision_table));
    ...
    if decision_table.annotations().len() > 0 {
        ...
        html_second_vertical_double_line
            .set_style(&prepare_style_second_vertical_double_line(decision_table));
    ...
    if decision_table.output_clauses().len() > 1 {
        if let Some(output_label) = &decision_table.output_label() {
            ...
            html_output_label.set_style(&prepare_style_output_label(
                decision_table.output_clauses().len(),
            ));
...
fn prepare_style_horizontal_double_line(decision_table: &DecisionTable) -> String {
    ...
    format!(
        "grid-column: 1 / span {}; grid-row: {} / span 1;",
        column_count, row_start
    )
}
...
fn prepare_style_first_vertical_double_line(decision_table: &DecisionTable) -> String {
    ...
    format!(
        "grid-column: {} / span 1; grid-row: 1 / span {};",
        col_start, row_count
    )
}
...
fn prepare_style_second_vertical_double_line(decision_table: &DecisionTable) -> String {
    let row_count = get_row_count(decision_table) + decision_table.rules().len();
    let col_start =
        3 + decision_table.input_clauses().len() + decision_table.output_clauses().len();
    format!(
        "grid-column: {} / span 1; grid-row: 1 / span {};",
        col_start, row_count
    )
...
fn get_column_count(decision_table: &DecisionTable) -> usize {
    let mut column_count = 2
        + decision_table.input_clauses().len()
        + decision_table.output_clauses().len()
        + decision_table.annotations().len();
    if decision_table.annotations().len() > 0 {
        column_count += 1;
    }
    column_count
