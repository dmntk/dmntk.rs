...
pub fn dmn_model_to_html(definitions: &Definitions) -> String {
    ...
    // add model diagrams
    body.add_child(create_html_heading(
        HeadingLevel::H2,
        HEADING_MODEL_DIAGRAMS,
    ));
    ...
    HtmlDocument::new(
        document_title,
        "en",
        &[DMN_MODEL_CSS, DECISION_TABLE_CSS, &diagram_shared_styles],
        body,
    )
    .to_string()
}
    ...
    if let Some(dmndi) = definitions.dmndi() {
        for style in &dmndi.styles {
            if let Some(style_id) = &style.id {
                let mut css_class =
                    CssClass::new(format!("{}{}", DIAGRAM_SHARED_STYLE_PREFIX, style_id));
                if style.font_bold {
...
pub fn decision_table_to_html(decision_table: &DecisionTable) -> String {
    ...
    HtmlDocument::new(
        document_title,
        "en",
        &[DMN_MODEL_CSS, DECISION_TABLE_CSS],
        body,
    )
    .to_string()
}
...
fn create_model_diagrams(definitions: &Definitions) -> Vec<HtmlElement> {
    ...
    if let Some(dmndi) = definitions.dmndi() {
        for diagram in &dmndi.diagrams {
                    ...
                    DmnDiagramElement::DmnShape(shape) => {
                        if let Some(dmn_element_ref) = &shape.dmn_element_ref {
                            if let Some(decision) =
                                definitions.get_decision(dmn_element_ref.as_str())
                            {
                                html_svg_content.push(create_svg_decision(shape, decision));
                            } else if let Some(input_data) =
                                definitions.get_input_data(dmn_element_ref.as_str())
                            {
                                html_svg_content.push(create_svg_input_data(shape, input_data));
                            } else if let Some(business_knowledge_model) =
                                definitions.get_business_knowledge_model(dmn_element_ref.as_str())
                            {
                                html_svg_content.push(create_svg_business_knowledge_model(
                                    shape,
                                    business_knowledge_model,
                                ));
                            } else if let Some(knowledge_source) =
                                definitions.get_knowledge_source(dmn_element_ref.as_str())
                            {
                                html_svg_content
                                    .push(create_svg_knowledge_source(shape, knowledge_source));
                            }
                        }
                    }
                            ...
                            if let Some(requirement) = definitions.get_requirement(id) {
                                match requirement {
                                    Requirement::Information(_) => {
                                        html_svg_content.push(
                                            create_svg_edge_solid_with_black_arrow(
                                                &edge.way_points,
                                            ),
                                        );
                                    }
                                    Requirement::Knowledge(_) => {
                                        html_svg_content.push(
                                            create_svg_edge_dashed_with_thin_arrow(
                                                &edge.way_points,
                                            ),
                                        );
                                    }
                                    Requirement::Authority(_) => {
                                        html_svg_content.push(
                                            create_svg_edge_dashed_with_end_point(&edge.way_points),
                                        );
                                    }
                                }
                            }
            ...
            html_elements.push(create_svg_tag(
                &diagram.name,
                &diagram.size,
                html_svg_content,
            ));
...
fn create_model_element_decision_service(decision_service: &DecisionService) -> HtmlElement {
    ...
    let variable_details =
        create_variable_details(decision_service.variable(), HEADING_OUTPUT_DATA);
    //
...
fn create_model_element_decision(decision: &Decision) -> HtmlElement {
    ...
    model_element_container
        .add_child_opt(create_model_expression_instance(decision.decision_logic()));
...
fn create_model_expression_instance(
    opt_expression_instance: &Option<ExpressionInstance>,
) -> Option<HtmlElement> {
    if let Some(expression_instance) = opt_expression_instance {
        ...
        match expression_instance {
            ExpressionInstance::Context(_) => {
                let mut variable_details_heading =
                    HtmlElement::new_div(Some("variable-details-heading"));
            ...
            ExpressionInstance::LiteralExpression(literal_expression) => {
                ...
                container.add_child(variable_details_heading);
                let mut element_literal_expression =
                    HtmlElement::new_div(Some("literal-expression"));
                let no_literal_expression = "(no literal expression specified)".to_string();
                let text = literal_expression
                    .text()
                    .as_ref()
                    .unwrap_or(&no_literal_expression);
...
fn create_svg_decision(shape: &DmnShape, decision: &Decision) -> HtmlElement {
    ...
    let text = create_svg_text(
        ...
        get_label_text(shape, decision.name()),
    );
...
pub fn create_svg_business_knowledge_model(
    shape: &DmnShape,
...
fn create_svg_edge_solid_with_black_arrow(way_points: &[DcPoint]) -> HtmlElement {
    // prepare line
    let points = way_points
        .iter()
        .map(|w| format!("{},{} ", w.x, w.y))
        .collect::<String>();
...
fn create_svg_text(
    ...
    match (parent_style, label_style) {
        (Some(parent_class), Some(label_class)) => {
            span.set_attr("class", format!("{} {}", label_class, parent_class))
        }
    ...
    div.set_attr(
        "style",
        "display:table;height:100%;width:100%;text-align:center;",
    );
    div.add_child(span);
...
fn get_shared_class_name(shape: &DmnShape) -> Option<String> {
    if let Some(shared_style_id) = &shape.shared_style {
        return Some(format!(
            "{}{}",
            DIAGRAM_SHARED_STYLE_PREFIX, shared_style_id
        ));
    }
    None
}
...
pub fn create_svg_tag(
    title: &str,
    dimension: &Option<DcDimension>,
...
mod tests {
    ...
    fn test_create_html_heading() {
        assert_eq!(
            "<h1>Heading H1</h1>",
            create_html_heading(HeadingLevel::H1, "Heading H1").to_string()
        );
        assert_eq!(
            "<h2>Heading H2</h2>",
            create_html_heading(HeadingLevel::H2, "Heading H2").to_string()
        );
        assert_eq!(
            "<h3>Heading H3</h3>",
            create_html_heading(HeadingLevel::H3, "Heading H3").to_string()
        );
        assert_eq!(
            "<h4>Heading H4</h4>",
            create_html_heading(HeadingLevel::H4, "Heading H4").to_string()
        );
        assert_eq!(
            "<h5>Heading H5</h5>",
            create_html_heading(HeadingLevel::H5, "Heading H5").to_string()
        );
        assert_eq!(
            "<h6>Heading H6</h6>",
            create_html_heading(HeadingLevel::H6, "Heading H6").to_string()
        );
    }
}
