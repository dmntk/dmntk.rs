...
impl TryFrom<&ValueDto> for Value {
    ...
    fn try_from(value: &ValueDto) -> Result<Self, Self::Error> {
        ...
        Err(err_missing_attribute(
            "no 'simple', 'components' or 'list' attribute",
        ))
    }
}
    ...
    fn try_from(value: &SimpleDto) -> Result<Self, Self::Error> {
        ...
        if let Some(typ) = &value.typ {
            if let Some(text) = &value.text {
                ...
                Err(err_missing_attribute(
                    "simple value must have 'text' attribute",
                ))
            }
        } else {
            Err(err_missing_attribute(
                "simple value must have 'type' attribute",
            ))
        }
    }
}
    ...
    fn try_from(object: &ObjectDto) -> Result<Self, Self::Error> {
        ...
        for item in object {
            let item_name = item
                .name
                .as_ref()
                .ok_or_else(|| err_invalid_attribute("component must have a name"))?
                .to_string();
