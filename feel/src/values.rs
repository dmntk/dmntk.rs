...
impl fmt::Display for Value {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Value::FunctionBody(_, external) => write!(
                f,
                "FunctionBody{}",
            ...
            Value::Null(trace) => write!(
                f,
                "null{}",
                trace.as_ref().map_or("".to_string(), |s| format!("({s})"))
            ),
...
impl Value {
        ...
        match self {
            ...
            Value::FunctionDefinition(parameters, _, _, _, _, result_type) => {
                let parameter_types = parameters
                    .iter()
                    .map(|(_, feel_type)| feel_type.clone())
                    .collect();
    ...
    pub fn try_from_xsd_integer(text: &str) -> Result<Self> {
        let value = text
            .parse::<FeelNumber>()
            .map_err(|_| err_invalid_xsd_integer(text))?;
    ...
    pub fn try_from_xsd_decimal(text: &str) -> Result<Self> {
        let value = text
            .parse::<FeelNumber>()
            .map_err(|_| err_invalid_xsd_decimal(text))?;
        Ok(Value::Number(value))
    }
    ...
    pub fn try_from_xsd_double(text: &str) -> Result<Self> {
        let value = text
            .parse::<FeelNumber>()
            .map_err(|_| err_invalid_xsd_double(text))?;
        Ok(Value::Number(value))
    }
    ...
    pub fn try_from_xsd_date_time(text: &str) -> Result<Self> {
        Ok(Value::DateTime(
            FeelDateTime::try_from(text).map_err(|_| err_invalid_xsd_date_time(text))?,
        ))
    }
...
pub fn values_to_feel_string(values: &Values) -> String {
    format!(
        ...
        values
            .iter()
            .map(|value| value.to_feel_string())
            .collect::<Vec<String>>()
            .join(", ")
    )
}
