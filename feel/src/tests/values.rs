...
fn test_debug() {
    ...
    eq_dbg!(
        r#"BuiltInFunction(Time)"#,
        Value::BuiltInFunction(Bif::Time)
    );
    eq_dbg!(
        r#"Context(FeelContext({}))"#,
        Value::Context(FeelContext::default())
    );
    eq_dbg!(
        r#"ContextEntry(Name("a"), Number(+1E+0))"#,
        Value::ContextEntry(name.clone(), b_number.clone())
    );
    eq_dbg!(
        r#"ContextEntryKey(Name("a"))"#,
        Value::ContextEntryKey(name.clone())
    );
    eq_dbg!(
        r#"ContextType(Number)"#,
        Value::ContextType(t_number.clone())
    );
    eq_dbg!(
    ...
    eq_dbg!(
        r#"ExpressionList([])"#,
        Value::ExpressionList(Values::default())
    );
    eq_dbg!(
    ...
    eq_dbg!(
        r#"FormalParameter(Name("a"), Number)"#,
        Value::FormalParameter(name.clone(), t_number.clone())
    );
    ...
    eq_dbg!(
        r#"IntervalEnd(Number(+1E+0), false)"#,
        Value::IntervalEnd(b_number.clone(), false)
    );
    eq_dbg!(
        r#"IntervalStart(Number(+1E+0), false)"#,
        Value::IntervalStart(b_number.clone(), false)
    );
    ...
    eq_dbg!(
        ...
        Value::NamedParameter(
            Box::new(Value::ParameterName(name.clone())),
            b_number.clone()
        )
    );
    eq_dbg!(
        r#"NamedParameters({})"#,
        Value::NamedParameters(BTreeMap::new())
    );
    eq_dbg!(
        r#"NegatedCommaList([])"#,
        Value::NegatedCommaList(Values::default())
    );
    eq_dbg!(r#"Number(+1E+0)"#, Value::Number(FeelNumber::new(1, 0)));
    eq_dbg!(r#"Null(None)"#, Value::Null(None));
    eq_dbg!(
        r#"ParameterName(Name("a"))"#,
        Value::ParameterName(name.clone())
    );
    eq_dbg!(r#"ParameterTypes([])"#, Value::ParameterTypes(vec![]));
    eq_dbg!(
        r#"PositionalParameters([])"#,
        Value::PositionalParameters(Values::default())
    );
    eq_dbg!(
        r#"QualifiedNameSegment(Name("a"))"#,
        Value::QualifiedNameSegment(name)
    );
    eq_dbg!(
    ...
    eq_dbg!(
        r#"Time(FeelTime(12, 13, 23, 0, Local))"#,
        Value::Time(v_time)
    );
    eq_dbg!(
        r#"UnaryGreater(Number(+1E+0))"#,
        Value::UnaryGreater(b_number.clone())
    );
    eq_dbg!(
        r#"UnaryGreaterOrEqual(Number(+1E+0))"#,
        Value::UnaryGreaterOrEqual(b_number.clone())
    );
    eq_dbg!(
        r#"UnaryLess(Number(+1E+0))"#,
        Value::UnaryLess(b_number.clone())
    );
    eq_dbg!(
        r#"UnaryLessOrEqual(Number(+1E+0))"#,
        Value::UnaryLessOrEqual(b_number)
    );
    eq_dbg!(
...
fn test_type_of() {
    ...
    eq_typ!(
        FeelType::List(Box::new(t_number)),
        Value::List(vec![v_number.clone()])
    );
    eq_typ!(
        FeelType::List(Box::new(FeelType::Any)),
        Value::List(vec![v_number, v_boolean])
    );
    eq_typ!(
        FeelType::Any,
        Value::NamedParameter(
    ...
    eq_typ!(
        ...
        Value::Range(b_number.clone(), false, b_boolean, true)
    );
...
fn test_comparison() {
    ...
    let fun_body_e =
        FunctionBody::DecisionTable(Arc::new(Box::new(|_: &FeelScope| value_number!(4))));
    let fun_body_f =
        FunctionBody::DecisionTable(Arc::new(Box::new(|_: &FeelScope| value_number!(5))));
...
fn test_value_to_feel_string() {
    assert_eq!(
        r#""foo""#,
        Value::String("foo".to_string()).to_feel_string()
    );
    assert_eq!(
        r#""\"bar\"""#,
        Value::String("\"bar\"".to_string()).to_feel_string()
    );
    assert_eq!(
        r#"{}"#,
        Value::Context(FeelContext::default()).to_feel_string()
    );
    assert_eq!(r#"[]"#, Value::List(Values::default()).to_feel_string());
    assert_eq!(
        r#"[1.2, 2.1]"#,
        Value::List(vec![
            Value::Number(FeelNumber::new(12, 1)),
            Value::Number(FeelNumber::new(21, 1))
        ])
        .to_feel_string()
    );
...
fn test_jsonify() {
    ...
    assert_eq!(
        r#""2023-02-24""#,
        Value::Date(FeelDate::new(2023, 2, 24)).jsonify()
    );
    assert_eq!(
        r#""18:35:12.000598677Z""#,
        Value::Time(FeelTime::utc(18, 35, 12, 598677)).jsonify()
    );
    assert_eq!(
        r#""2023-02-24T18:35:12.000598677Z""#,
        Value::DateTime(FeelDateTime::new(
            FeelDate::new(2023, 2, 24),
            FeelTime::utc(18, 35, 12, 598677)
        ))
        .jsonify()
    );
    assert_eq!(
        r#""P10DT13H59M18S""#,
        Value::DaysAndTimeDuration(FeelDaysAndTimeDuration::from_s(914358)).jsonify()
    );
    assert_eq!(
        r#""P3Y2M""#,
    ...
    assert_eq!(
        r#""null(error details)""#,
        Value::Null(Some("error details".to_string())).jsonify()
    );
    let n_start = Box::new(Value::Number(FeelNumber::new(1, 0)));
    let n_end = Box::new(Value::Number(FeelNumber::new(12, 0)));
    assert_eq!(
        r#""(1..12]""#,
        Value::Range(n_start, false, n_end, true).jsonify()
    );
    assert_eq!(
        r#"jsonify trait not implemented for value: Irrelevant"#,
        Value::Irrelevant.jsonify()
    );
}
...
fn test_try_from() {
    ...
    assert_eq!(
        ...
        Value::try_from_xsd_double("1.2a")
            .err()
            .unwrap()
            .to_string()
    );
    ...
    assert_eq!(
        ...
        Value::try_from_xsd_boolean("TRUE")
            .err()
            .unwrap()
            .to_string()
    );
    ...
    assert_eq!(
        ...
        Value::try_from_xsd_date("2022-02-31")
            .err()
            .unwrap()
            .to_string()
    );
    ...
    assert_eq!(
        ...
        Value::try_from_xsd_time("26:12:34")
            .err()
            .unwrap()
            .to_string()
    );
    ...
    assert_eq!(
        ...
        Value::try_from_xsd_date_time("2022-02-31T23:12:34")
            .err()
            .unwrap()
            .to_string()
    );
    ...
    assert_eq!(
        ...
        Value::try_from_xsd_duration("PYD")
            .err()
            .unwrap()
            .to_string()
    );
...
fn test_coerced() {
    ...
    assert_eq!(
        ...
        v_list_number_2.coerced(T_NUMBER).to_string()
    );
    assert_eq!(
        r#"null(after coercion)"#,
        v_list_string_1.coerced(T_NUMBER).to_string()
    );
    ...
    assert_eq!(
        r#"null(after coercion)"#,
        v_irrelevant.coerced(T_NUMBER).to_string()
    );
    assert_eq!(
        r#"null(after coercion)"#,
        v_context_d.coerced(&T_CONTEXT_A).to_string()
    );
}
