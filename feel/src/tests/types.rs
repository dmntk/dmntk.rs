...
fn test_from_string() {
    ...
    assert_eq!(
        FeelType::DateTime,
        FeelType::from_str("date and time").unwrap()
    );
    assert_eq!(
        FeelType::DaysAndTimeDuration,
        FeelType::from_str("days and time duration").unwrap()
    );
    assert_eq!(
        FeelType::YearsAndMonthsDuration,
        FeelType::from_str("years and months duration").unwrap()
    );
    assert_eq!(
        "<TypesError> invalid FEEL type name: range",
        FeelType::from_str("range").err().unwrap().to_string()
    );
}
...
fn test_get_value_checked() {
    ...
    assert_eq!(
        "null",
        T_ANY.get_value_checked(&v_null).unwrap().to_string()
    );
    assert_eq!(
        "true",
        T_ANY.get_value_checked(&v_boolean).unwrap().to_string()
    );
...
fn test_type_stringify() {
    ...
    assert_eq!(
        "days and time duration",
        T_DAYS_AND_TIME_DURATION.to_string()
    );
    assert_eq!(
        "years and months duration",
        T_YEARS_AND_MONTHS_DURATION.to_string()
    );
    ...
    assert_eq!(
        "context<a: number, b: boolean, c: string>",
        T_CONTEXT_A_B_C.to_string()
    );
}
...
fn test_type_get_conformant_value() {
    ...
    // date time
    let v_date_time = Value::DateTime(FeelDateTime::new(
        FeelDate::new(2022, 9, 27),
        FeelTime::local_opt(9, 2, 0, 0).unwrap(),
    ));
    // days and time duration
    ...
    // years and months duration
    let v_years_and_months_duration =
    ...
    let v_function_a = Value::FunctionDefinition(
        vec![
            (NAME_A.clone(), T_NUMBER.clone()),
            (NAME_B.clone(), T_NUMBER.clone()),
        ],
    ...
    // ranges
    let v_range_a = Value::Range(
        Box::new(v_number_1.clone()),
        false,
        Box::new(v_number_2),
        false,
    );
