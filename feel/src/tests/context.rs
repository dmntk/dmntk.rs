...
fn test_context_two_levels() {
    ...
    assert_eq!(
        "{age: 49, b: {married: true}, x y: true}",
        ctx_a.to_string()
    );
    ...
    assert_eq!(
        "49",
        ctx_a.get_entry(&name_age).unwrap().to_string().as_str()
    );
    assert_eq!(
        "{married: true}",
        ctx_a.get_entry(&name_b).unwrap().to_string().as_str()
    );
}
...
fn test_context_three_levels() {
    ...
    ctx_a.set_entry(&name_b, ctx_b.into());
    assert_eq!(
        r#"{age: 49, b: {c: {car: "opel"}, married: true}}"#,
        ctx_a.to_string()
    );
    assert!(ctx_a.contains_entry(&name_age));
    assert!(ctx_a.contains_entry(&name_b));
...
fn test_context_search_entry() {
    ...
    assert_eq!(
        "{married: true}",
        ctx_a.search_entry(&qn_b).unwrap().to_string().as_str()
    );
    let qn_b_married = QualifiedName::new(&[&name_b, &name_married]);
    assert_eq!(
        "true",
        ctx_a
            .search_entry(&qn_b_married)
            .unwrap()
            .to_string()
            .as_str()
    );
...
fn test_context_create_entry() {
    ...
    assert_eq!(
        "{b: {c: {d: true}}}",
        ctx.search_entry(&qn_a).unwrap().to_string().as_str()
    );
    assert_eq!(
        "{c: {d: true}}",
        ctx.search_entry(&qn_a_b).unwrap().to_string().as_str()
    );
    assert_eq!(
        "{d: true}",
        ctx.search_entry(&qn_a_b_c).unwrap().to_string().as_str()
    );
    assert_eq!(
        "true",
        ctx.search_entry(&qn_a_b_c_d).unwrap().to_string().as_str()
    );
    let mut ctx: FeelContext = Default::default();
    ...
    assert_eq!(
        r#"{a: true, b: false, c: {d: "deep"}}"#,
        ctx.to_string().as_str()
    );
    let mut ctx: FeelContext = Default::default();
...
fn test_get_entries() {
    ...
    assert_eq!(
        vec![(&name_a, &value_number!(1)), (&name_b, &value_number!(2))],
        ctx.get_entries()
    );
}
...
fn test_value_is_not_a_context() {
    assert_eq!(
        ...
        <Value as TryInto<FeelContext>>::try_into(Value::Boolean(true))
            .err()
            .unwrap()
            .to_string()
    );
