...
impl FeelDaysAndTimeDuration {
    ...
    pub fn get_seconds(&self) -> usize {
        ((self.0.abs() % NANOSECONDS_IN_DAY % NANOSECONDS_IN_HOUR % NANOSECONDS_IN_MINUTE)
            / NANOSECONDS_IN_SECOND) as usize
    }
...
impl fmt::Display for FeelDaysAndTimeDuration {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match (day > 0, hour > 0, minute > 0, seconds > 0, nanoseconds > 0) {
            ...
            (true, true, true, true, true) => write!(
                f,
                "{sign}P{day}DT{hour}H{minute}M{seconds}.{nanoseconds_str}S"
            ),
        }
    }
}
...
impl TryFrom<&str> for FeelDaysAndTimeDuration {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        Err(err_invalid_date_and_time_duration_literal(
            value.to_string(),
        ))
    }
}
...
mod tests {
    ...
    fn test_eq_should_pass() {
        assert_eq!(
            Some(Ordering::Equal),
            FeelDaysAndTimeDuration::from_sn(0, 0)
                .partial_cmp(&FeelDaysAndTimeDuration::from_sn(0, 0))
        );
        assert_eq!(
            Some(Ordering::Equal),
            FeelDaysAndTimeDuration::from_sn(0, 10)
                .partial_cmp(&FeelDaysAndTimeDuration::from_sn(0, 10))
        );
        assert_eq!(
            Some(Ordering::Equal),
            FeelDaysAndTimeDuration::from_sn(0, 999_999_999)
                .partial_cmp(&FeelDaysAndTimeDuration::from_sn(0, 999_999_999))
        );
        assert_eq!(
            Some(Ordering::Equal),
            FeelDaysAndTimeDuration::from_sn(86_400, 999_999_999)
                .partial_cmp(&FeelDaysAndTimeDuration::from_sn(86_400, 999_999_999))
        );
        assert_eq!(
            FeelDaysAndTimeDuration::from_n(0),
            FeelDaysAndTimeDuration::from_n(0)
        );
        assert_eq!(
            FeelDaysAndTimeDuration::from_n(0),
    ...
    fn test_lt_should_pass() {
        assert_eq!(
            Some(Ordering::Less),
            FeelDaysAndTimeDuration::from_s(10).partial_cmp(&FeelDaysAndTimeDuration::from_s(11))
        );
        assert_eq!(
            Some(Ordering::Less),
            FeelDaysAndTimeDuration::from_sn(10, 1)
                .partial_cmp(&FeelDaysAndTimeDuration::from_sn(10, 2))
        );
    ...
    fn test_gt_should_pass() {
        ...
        assert_eq!(
            Some(Ordering::Greater),
            FeelDaysAndTimeDuration::from_sn(10, 1)
                .partial_cmp(&FeelDaysAndTimeDuration::from_s(10))
        );
        assert!(FeelDaysAndTimeDuration::from_s(11) > FeelDaysAndTimeDuration::from_s(10));
