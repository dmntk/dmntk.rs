                                        ...
                                        if let Ok(mut hour) = hour_match.as_str().parse::<u8>() {
                                            if let Some(min_match) = captures.name("minutes") {
                                                if let Ok(min) = min_match.as_str().parse::<u8>() {
                                                    if let Some(sec_match) =
                                                        captures.name("seconds")
                                                    {
                                                        if let Ok(sec) =
                                                            sec_match.as_str().parse::<u8>()
                                                        {
                                                            let fractional =
                                                                if let Some(frac_match) =
                                                                    captures.name("fractional")
                                                                {
                                                                    frac_match
                                                                        .as_str()
                                                                        .parse::<f64>()
                                                            ...
                                                            let nanos =
                                                                (fractional * 1e9).trunc() as u64;
                                                            if let Some(mut date) =
                                                                FeelDate::new_opt(year, month, day)
                                                            {
                                                                if let Some(zone) =
                                                                    FeelZone::from_captures(
                                                                        &captures,
                                                                    )
                                                                {
                                                                    if !is_valid_time(
...
impl PartialEq for FeelDateTime {
    ...
    fn eq(&self, rhs: &Self) -> bool {
        ...
        let equal_zones = match (lhs_zone, rhs_zone) {
            ...
            (FeelZone::Utc, FeelZone::Zone(zone_name))
            | (FeelZone::Zone(zone_name), FeelZone::Utc) => zone_name == ETC_UTC,
            (FeelZone::Utc, FeelZone::Offset(offset))
            | (FeelZone::Offset(offset), FeelZone::Utc) => *offset == 0,
            (FeelZone::Local, FeelZone::Local) => true,
            (FeelZone::Offset(offset), FeelZone::Zone(zone_name))
            | (FeelZone::Zone(zone_name), FeelZone::Offset(offset)) => {
                *offset == 0 && zone_name == ETC_UTC
            }
            (FeelZone::Offset(_), FeelZone::Offset(_)) => true,
            (FeelZone::Zone(zone_name1), FeelZone::Zone(zone_name2)) => {
                (zone_name1 == zone_name2)
                    || (get_zone_offset(zone_name1) == get_zone_offset(zone_name2))
            }
            _ => return false,
        };
        if !equal_zones {
        ...
        let lhs_time_tuple = (
            (self.1).hour() as u32,
            (self.1).minute() as u32,
            (self.1).second() as u32,
            0,
        );
        let lhs_offset_opt = match lhs_zone {
            ...
            FeelZone::Zone(zone_name) => {
                get_zone_offset_dt(zone_name, lhs_date_tuple, lhs_time_tuple)
            }
        };
        ...
        let rhs_offset_opt = match rhs_zone {
            ...
            FeelZone::Offset(offset) => Some(*offset),
            FeelZone::Zone(zone_name) => {
                get_zone_offset_dt(zone_name, rhs_date_tuple, rhs_time_tuple)
            }
        };
...
impl Add<FeelDaysAndTimeDuration> for FeelDateTime {
    ...
    fn add(self, rhs: FeelDaysAndTimeDuration) -> Self::Output {
        ...
        if let Ok(fixed_date_time) =
            <FeelDateTime as TryInto<DateTime<FixedOffset>>>::try_into(self)
        {
            if let Some(date_time) =
                fixed_date_time.checked_add_signed(Duration::nanoseconds(rhs.as_nanos()))
            {
...
impl Sub<FeelDateTime> for FeelDateTime {
    ...
    fn sub(self, other: FeelDateTime) -> Self::Output {
        ...
        let me_offset_opt = match me_zone {
            ...
            FeelZone::Offset(offset) => Some(*offset),
            FeelZone::Zone(zone_name) => {
                get_zone_offset_dt(zone_name, me_date_tuple, me_time_tuple)
            }
        };
        let other_date_tuple = other.0.as_tuple();
        let other_time_tuple = (
            (other.1).hour() as u32,
            (other.1).minute() as u32,
            (other.1).second() as u32,
            (other.1).nanos() as u32,
        );
        let other_offset_opt = match other_zone {
            ...
            FeelZone::Offset(offset) => Some(*offset),
            FeelZone::Zone(zone_name) => {
                get_zone_offset_dt(zone_name, other_date_tuple, other_time_tuple)
            }
        };
        if let Some((me_offset, other_offset)) = me_offset_opt.zip(other_offset_opt) {
            ...
            let other_date_opt =
                date_time_offset_dt(other_date_tuple, other_time_tuple, other_offset);
...
impl TryFrom<FeelDateTime> for DateTime<FixedOffset> {
    ...
    fn try_from(value: FeelDateTime) -> Result<Self, Self::Error> {
        ...
        let me_time_tuple = (
            (value.1).hour() as u32,
            (value.1).minute() as u32,
            (value.1).second() as u32,
            (value.1).nanos() as u32,
        );
...
impl FeelDateTime {
    ...
    pub fn utc(
        ...
        Self(
            FeelDate::new(year, month, day),
            FeelTime::utc(hour, minute, second, nanosecond),
        )
    }
    ...
    pub fn local(
        ...
        Self(
            FeelDate::new(year, month, day),
            FeelTime::local(hour, min, sec, nanos),
        )
    }
    ...
    pub fn offset(date: (Year, Month, Day), time: (u8, u8, u8, u64), offset: i32) -> Self {
        Self(
            FeelDate::new(date.0, date.1, date.2),
            FeelTime::offset(time.0, time.1, time.2, time.3, offset),
        )
    }
