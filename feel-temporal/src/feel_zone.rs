...
mod tests {
    ...
    macro_rules! assert_zone {
        ($zone:expr, $time:expr) => {
            assert_eq!(
                $zone,
                FeelZone::from_captures(&RE_TIME.captures($time).unwrap())
            );
        };
    }
    ...
    fn test_format_offset() {
        ...
        assert_eq!(
            "-05:00:01",
            FeelZone::try_from(-18_001).unwrap().to_string()
        );
        assert_eq!("-05:00:01", FeelZone::Offset(-18_001).to_string());
    ...
    fn test_from_captures() {
        ...
        assert_zone!(None, "00:00:00-14:00:01");
        assert_zone!(
            Some(FeelZone::Zone("Europe/Warsaw".to_string())),
    ...
    fn test_eq() {
        ...
        assert!(
            (FeelZone::Zone("Europe/Warsaw".to_string())
                != FeelZone::Zone("Australia/Sydney".to_string()))
        );
    }
    ...
    fn test_debug() {
        ...
        assert_eq!(
            r#"Offset(18000)"#,
            format!("{:?}", FeelZone::Offset(18_000))
        );
        assert_eq!(
            r#"Zone("Europe/Warsaw")"#,
            format!("{:?}", FeelZone::Zone("Europe/Warsaw".to_string()))
        );
    }
}
