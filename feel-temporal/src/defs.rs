...
const DATE_PATTERN: &str =
    r#"(?P<sign>-)?(?P<year>[1-9][0-9]{3,8})-(?P<month>[0-9]{2})-(?P<day>[0-9]{2})"#;
/// Regular expression pattern for parsing time.
const TIME_PATTERN: &str =
    r#"(?P<hours>[0-9]{2}):(?P<minutes>[0-9]{2}):(?P<seconds>[0-9]{2})(?P<fractional>\.[0-9]+)?"#;
...
pub static TIME_ZONE_PATTERN: Lazy<String> =
    Lazy::new(|| format!("{ZULU_PATTERN}|{ZONE_PATTERN}|{OFFSET_PATTERN}"));
...
pub static RE_TIME: Lazy<Regex> = Lazy::new(|| {
    Regex::new(&format!(
        "^{TIME_PATTERN}({})?$",
        TIME_ZONE_PATTERN.as_str()
    ))
    .unwrap()
});
/// Regular expression for parsing date and time.
pub static RE_DATE_AND_TIME: Lazy<Regex> = Lazy::new(|| {
    Regex::new(&format!(
...
///
pub fn date_time_offset_dt(
    date: (i32, u32, u32),
    time: (u32, u32, u32, u32),
    offset: i32,
) -> Option<DateTime<FixedOffset>> {
    if let Some(fixed_offset) = FixedOffset::east_opt(offset) {
        if let LocalResult::Single(offset_date_time) =
            fixed_offset.with_ymd_and_hms(date.0, date.1, date.2, time.0, time.1, time.2)
        {
...
pub fn get_local_offset_dt(date: (i32, u32, u32), time: (u32, u32, u32, u32)) -> Option<i32> {
    if let Some(naive_date) = NaiveDate::from_ymd_opt(date.0, date.1, date.2) {
        if let Some(naive_time) = NaiveTime::from_hms_nano_opt(time.0, time.1, time.2, time.3) {
            ...
            return Some(
                Local
                    .offset_from_utc_datetime(&naive_date_time)
                    .local_minus_utc(),
            );
        }
    }
...
/// and UTC time zone at specified **date and time**.
pub fn get_zone_offset_dt(
    zone_name: &str,
    date: (i32, u32, u32),
    time: (u32, u32, u32, u32),
) -> Option<i32> {
    // try to build UTC date and time from specified values
    if let LocalResult::Single(utc_date_time) =
        Utc.with_ymd_and_hms(date.0, date.1, date.2, time.0, time.1, time.2)
    {
        if let Some(utc) = utc_date_time.with_nanosecond(time.3) {
            ...
            if let Ok(tz) = zone_name.parse::<chrono_tz::Tz>() {
                ...
                if let LocalResult::Single(z_date_time) =
...
mod tests {
    ...
    fn test_get_local_offset_dt() {
        ...
        let warsaw_offset =
            get_zone_offset_dt("Europe/Warsaw", (2020, 6, 12), (9, 12, 3, 0)).unwrap();
        let local_offset = get_local_offset_dt((2020, 6, 12), (9, 12, 3, 0)).unwrap();
        if local_offset == warsaw_offset {
            assert_eq!(
                Some(SECONDS_IN_HOUR),
                get_local_offset_dt((2020, 10, 29), (9, 12, 3, 0))
            );
            assert_eq!(
    ...
    fn test_get_zone_offset() {
        ...
        assert_eq!(
            Some(SECONDS_IN_HOUR),
            get_zone_offset_dt("Europe/Warsaw", (2020, 10, 29), (9, 12, 3, 0))
        );
        // summer time in Warsaw, offset = +02:00
        assert_eq!(
            Some(2 * SECONDS_IN_HOUR),
            get_zone_offset_dt("Europe/Warsaw", (2020, 6, 21), (11, 13, 49, 0))
        );
        // time in Moscow, offset = +03:00
        assert_eq!(
            Some(3 * SECONDS_IN_HOUR),
            get_zone_offset_dt("Europe/Moscow", (2020, 10, 29), (9, 12, 3, 0))
        );
        ...
        assert_eq!(
            Some(-10 * SECONDS_IN_HOUR),
            get_zone_offset_dt("Pacific/Honolulu", (2020, 6, 8), (8, 0, 0, 0))
        );
    }
    ...
    fn test_feel_time_offset() {
        ...
        assert_eq!(
            0,
            feel_time_offset(&FeelDateTime::new(feel_date.clone(), feel_time_a)).unwrap()
        );
        let feel_time_b = FeelTime::local(0, 0, 0, 0);
        assert_eq!(
            None,
            feel_time_offset(&FeelDateTime::new(feel_date.clone(), feel_time_b))
        );
        let feel_time_c = FeelTime::offset(0, 0, 0, 0, 60);
        assert_eq!(
            60,
            feel_time_offset(&FeelDateTime::new(feel_date.clone(), feel_time_c)).unwrap()
        );
        let feel_time_d = FeelTime::zone_opt(0, 0, 0, 0, FeelZone::Utc).unwrap();
        assert_eq!(
            0,
            feel_time_offset(&FeelDateTime::new(feel_date.clone(), feel_time_d)).unwrap()
        );
        let feel_time_e = FeelTime::zone_opt(0, 0, 0, 0, FeelZone::Offset(15)).unwrap();
        assert_eq!(
            15,
            feel_time_offset(&FeelDateTime::new(feel_date.clone(), feel_time_e)).unwrap()
        );
        let feel_time_f =
            FeelTime::zone_opt(0, 0, 0, 0, FeelZone::Zone("Etc/UTC".to_string())).unwrap();
        assert_eq!(
            0,
            feel_time_offset(&FeelDateTime::new(feel_date, feel_time_f)).unwrap()
        );
    }
